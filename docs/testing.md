...
## Hardening coverage (2026-08-31, phase 2)

Added suites cover: physical device binding and cross-device evidence isolation (`device-binding.test.ts`); poisoned-localStorage and imported-bundle trust boundaries (`trust-boundaries.test.ts`); operational trust vs investigative state (`operational-trust.test.ts`); the derived static-viability aggregator including the ≥15 s measured-hold requirement and single-vs-pair claim independence (`static-viability.test.ts`); the T0/T1/T2 timing model, measured-only verification, stayTime rejection semantics, and domain-layer observation validation (`timing-model.test.ts`); native-static-first recommendation ordering and conflict-driven revalidation (`recommendations.test.ts`); report trust/conflict/assessment/timing content (`report-content.test.ts`); session-resolved behavior activation rules (`session-behavior.test.ts`); guided-test profile applicability with a synthetic CoolLEDUX profile; transfer-dismissal protection and abandoned-test evidence (store tests); and the two-item primary navigation.

## The physical sequence MatrixSmith now recommends

With the shipped iLedHat evidence, reconnecting the display and opening Investigate recommends the NATIVE static path first: (1) measure how long a static image stays still — the baseline Graffiti timing/viability run (mode=0, speed=0, stayTime=3) with the measured T0/T1/T2 timeline; (2) if it moves, the stayTime=0 discriminator (identical program, one changed byte); (3) if stayTime=0 holds still, static-image black behavior, then the pixel-channel probe; if both timing variants move, the pixel-channel probe next (needed for correct rendering on any path); (4) the Animation-as-static fallback only once Graffiti is conclusively non-viable and channels are characterized; then optional color/white quality, GIF, power-cycle, and recovery work. The app itself surfaces each next test — the order is derived from the strategy-viability requirement graph, not hardcoded.
