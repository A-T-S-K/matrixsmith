...
async function executeContentPlan(): Promise<{
  plan: TransmissionPlan;
  result: Awaited<ReturnType<TransmissionExecutor["execute"]>>;
}> {
...
describe("executor timing instrumentation", () => {
  it("records real per-packet write timestamps, not one shared plan timestamp", async () => {
    ...
    const startTimes = result.packetTimings.map((timing) =>
      Date.parse(timing.writeStartedAt),
    );
    ...
    for (let index = 1; index < startTimes.length; index += 1)
      expect(startTimes[index]!).toBeGreaterThanOrEqual(startTimes[index - 1]!);
  }, 30000);
  ...
  it("measures per-packet gaps and preserves the scheduled pacing separately", async () => {
    ...
    const laterGaps = result.packetTimings
      .slice(1)
      .map((timing) => timing.gapSincePreviousTxMs);
    ...
    expect(
      laterGaps.filter((gap) => (gap ?? 0) >= 45).length,
    ).toBeGreaterThanOrEqual(laterGaps.length - 1);
    for (const timing of result.packetTimings)
      expect(timing.scheduledDelayMs).toBe(60);
  }, 30000);

  it("reports the final host-accepted write timestamp for observation timers", async () => {
    const { result } = await executeContentPlan();
    expect(result.finalWriteAcceptedAt).toBe(
      result.packetTimings[result.packetTimings.length - 1]!.hostAcceptedAt,
    );
  }, 30000);
});
