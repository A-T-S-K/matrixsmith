...
describe("transfer wake lock", () => {
  it("acquires and releases without owning transfer success", async () => {
    ...
    const handle: WakeLockHandle = { release };
    const request = vi.fn(async () => handle);
    const lock = new TransferWakeLock({ wakeLock: { request } });
  ...
  it("treats unsupported or rejected requests as non-fatal", async () => {
    ...
    const lock = new TransferWakeLock({
      wakeLock: {
        request: async () => {
  ...
  it("reacquires after visibility returns when the browser releases it", async () => {
    ...
    const documentLike = {
      ...
      addEventListener: (
        ...
        listeners.set("visibilitychange", fn);
      },
    ...
    const lock = new TransferWakeLock({ wakeLock: { request } }, documentLike);
    await lock.acquire();
    released?.();
