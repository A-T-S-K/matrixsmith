...
describe("executor-owned pacing", () => {
  it("waits the declared delay between paced packets and skips it after the last", async () => {
    ...
    transport.write = async (endpoint, bytes, mode) => { timestamps.push(Date.now()); return originalWrite(endpoint, bytes, mode); };
    ...
    expect(gaps.filter((gap) => gap >= 45).length).toBeGreaterThanOrEqual(packetCount - 2);
  }, 30000);
});
