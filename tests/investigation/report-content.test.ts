...
async function connectedController(fingerprint: DeviceFingerprint = knownIledHatFingerprint()): Promise<MatrixController> {
...
const timer = (fieldId: string, milliseconds: number): ObservationValue => ({ kind: "duration", fieldId, milliseconds, measuredBy: "matrixsmith-timer" });
...
describe("investigation report content", () => {
  it("includes the structured live advertisement observation labeled by source", async () => {
    const fingerprint: DeviceFingerprint = {
      ...
      advertisementObservation: {
        ...
        name: "iLedHat", rssi: -52, txPower: 4,
        advertisedServiceUuids: ["0000fff0-0000-1000-8000-00805f9b34fb"],
        manufacturerData: [{ companyId: 0x31ae, dataHex: "5EEA07000001100020031E" }],
        serviceData: [],
      },
    };
  ...
  it("never fabricates a structured observation when none was captured", async () => {
    ...
    const report = controller.investigationReportMarkdown();
    expect(report).toContain("Structured advertisement observation: not captured");
  ...
  it("distinguishes trusted basis from historical conflict per claim", async () => {
    ...
    expect(report).toMatch(/animation\.frames[\s\S]*previous local session: REJECTED/);
    expect(report).toMatch(/animation\.frames[\s\S]*Operational basis: built-in profile evidence \(trusted\)/);
  });
  ...
  it("compares stayTime runs side by side once both exist", async () => {
    ...
    controller.recordGuidedTestObservations("coolledux-graffiti-staytime", [
      { kind: "boolean", fieldId: "initial-correct", value: "yes" },
      timer("image-visible", 1300), { kind: "boolean", fieldId: "moved", value: "no" }, timer("observation-end", 17300),
    ], []);
    const report = controller.investigationReportMarkdown();
  ...
  it("makes an encoder permutation exceptionally clear for AI-driven repair", async () => {
    ...
    controller.recordGuidedTestObservations("coolledux-pixel-channels", [
      patch(0x0000, "off"), patch(0x0f00, "green"), patch(0x00f0, "red"), patch(0x000f, "blue"), patch(0x0fff, "tinted-white"),
    ...
    expect(controller.claims().find((claim) => claim.id === "pixel.channel-map")?.status).toBe("verified");
    expect(controller.claims().find((claim) => claim.id === "pixel.encoder-correctness")?.status).toBe("rejected");
