...
} from "../../src/investigation/regions";
import {
  diagnosticContent,
...
const PROFILE = {
  width: iledHat31aeProfile.width,
  height: iledHat31aeProfile.height,
};
...
describe("diagnostic region validation", () => {
  ...
  it("requires a human-readable label and a map label", () => {
    expect(validateRegions([region({ displayLabel: "" })]).join(" ")).toContain(
      "no human-readable displayLabel",
    );
    expect(validateRegions([region({ shortLabel: " " })]).join(" ")).toContain(
      "no shortLabel",
    );
  });
  ...
  it("rejects regions that fall outside the panel", () => {
    expect(
      validateRegions([region({ x: 30, width: 8 })], PROFILE).join(" "),
    ).toContain("extends past the 32×16 panel");
  });

  it("rejects a question referencing an unknown region", () => {
    expect(
      validateRegionReferences(["nope"], [region({ id: "a" })]).join(" "),
    ).toContain('unknown region "nope"');
    expect(
      validateRegionReferences([undefined, "a"], [region({ id: "a" })]),
    ).toEqual([]);
  });

  it("resolves group peers so a grouped question can highlight every member", () => {
    const regions = [
      region({ id: "a", groupId: "g" }),
      region({ id: "b", groupId: "g" }),
      region({ id: "c" }),
    ];
...
describe("CoolLEDUX diagnostic regions", () => {
  const built = [...new Set(ILEDHAT_GUIDED_TESTS.map((test) => test.operation))]
    ...
    .map((operation) => ({
      operation,
      content: diagnosticContent(operation.diagnosticId).build(
        PROFILE,
  ...
  it("declares structurally valid, uniquely identified regions for every diagnostic", () => {
    for (const { operation, content } of built) {
      expect(
        validateRegions(content.regions, PROFILE),
        `regions for ${operation.diagnosticId}`,
      ).toEqual([]);
    }
  });
      ...
      const variants: Readonly<Record<string, number>>[] =
        definition.parameters.length === 0
          ...
          : definition.parameters.flatMap((parameter) =>
              parameter.allowed.map((value) => ({ [parameter.id]: value })),
            );
      const available = new Set(
        variants.flatMap((parameters) =>
          definition
            .build(PROFILE, parameters)
            .regions.map((entry) => entry.id),
        ),
      );
      const referenced = test.observation
        .map((spec) => spec.regionId)
        .filter((id): id is string => id !== undefined);
      for (const id of referenced)
        expect(available, `${test.id} references ${id}`).toContain(id);
    }
  });

  it("names channel zones for people and keeps the raw word as technical metadata", () => {
    const regions = diagnosticContent("pixel-channel-probe").build(
      PROFILE,
    ).regions;
    ...
    expect(regions.map((entry) => entry.displayLabel)).toContain(
      "Zone 2 · Red test",
    );
    expect(
      regions.find((entry) => entry.id === "channel-red")?.technical.rawWord,
    ).toBe(0x0f00);
  });
  ...
  it("keeps the exact raw word available for reports", () => {
    const regions = diagnosticContent("graffiti-black-probe").build(
      PROFILE,
    ).regions;
