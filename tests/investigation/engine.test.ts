...
function claimStatus(controller: ApplicationRuntime, claimId: string): string {
  return (
    controller.claims().find((claim) => claim.id === claimId)?.status ??
    "missing"
  );
}
...
describe("guided test engine", () => {
  it("lists driver-contributed tests with claim-based availability", async () => {
    ...
    expect(
      byId.get("coolledux-graffiti-staytime")?.missingCompletedTests,
    ).toEqual(["coolledux-graffiti-timing"]);
  ...
  it("refuses a guided transfer without explicit consequence confirmation", async () => {
    const controller = await connectedController();
    await expect(
      controller.runGuidedTestTransfer("coolledux-graffiti-black", {
        confirmedConsequence: false,
  ...
  it("transfers the diagnostic and reports the final host-accepted write time", async () => {
    const controller = await connectedController();
    const result = await controller.runGuidedTestTransfer(
    ...
    const transaction = controller.transactions.find(
      ({ id }) => id === result.transactionIds[0],
    );
  ...
  it("turns black-probe observations into current-session claim evidence", async () => {
    ...
    const shipped = controller
      .claims()
      .find((claim) => claim.id === "graffiti.black-semantics");
    ...
    expect(claimStatus(controller, "graffiti.black-semantics")).toBe(
      "verified",
    );
    ...
    expect(controller.investigation?.completedTests[0]?.testId).toBe(
      "coolledux-graffiti-black",
    );
  });
  ...
  it("keeps a movement observation as partial evidence with the measured onset", async () => {
    ...
    const values: ObservationValue[] = [
      ...
      {
        kind: "choice",
        fieldId: "motion-description",
        optionId: "wraps-repeats",
      },
    ];
    const completed = controller.recordGuidedTestObservations(
    ...
    expect(completed.established.join(" ")).toContain(
      "full raster appeared correctly",
    );
  ...
  it("derives the session raster strategy from full viability, not one passing test", () => {
    ...
    const open = [
      ...uncharacterizedCoolLedUxEvidence(),
      {
    ...
    const complete = [
      ...
      } as const,
      {
        ...
        scope: "current-session",
        provenance: "observed",
        summary: "encoder matches",
      } as const,
    ];
    expect(evaluateStaticViability(complete).selected).toBe(
  ...
  it("derives a usable strategy from the shipped profile with no session work at all", async () => {
    ...
    expect(controller.staticViability().selected).toBe(
      "animation-single-frame",
    );
    // …and normal content actually compiles through it.
    const plan = controller.plan({
      type: "ShowFrame",
      frame: (await import("../../src/render/patterns")).orientationPattern(
        32,
        16,
      ),
    });
  ...
  it("rejects the white-channel hypothesis when every high-nibble patch is off", async () => {
    ...
    expect(claimStatus(controller, "pixel.encoder-correctness")).toBe(
      "verified",
    );
    ...
    expect(
      controller
        .guidedTests()
        .find((entry) => entry.test.id === "coolledux-color-white")?.available,
    ).toBe(true);
  });
  ...
  it("treats a lit high-nibble patch as fourth-channel evidence without claiming RGBW", async () => {
    ...
    const values = [
      ...
      patch(0x1000, "off"),
      patch(0x2000, "off"),
      patch(0x4000, "neutral-white"),
      patch(0x8000, "neutral-white"),
      patch(0xf000, "neutral-white"),
      patch(0xffff, "neutral-white"),
    ];
    const completed = controller.recordGuidedTestObservations(
      "coolledux-pixel-channels",
      values,
    ...
    const evidence = controller.investigation?.claimEvidence.find(
      (entry) => entry.claimId === "pixel.fourth-channel",
    );
    expect(evidence?.summary).toContain(
      "does not by itself make the profile RGBW",
    );
  });

  it("supports symptom-driven troubleshooting investigations", async () => {
    const controller = await connectedController();
    const investigation = controller.startInvestigation({
      kind: "troubleshoot",
...
describe("evidence-aware guided operations", () => {
  it("omits high-nibble bands until a fourth channel is established", async () => {
    ...
    expect(
      (operation as { parameters?: Record<string, number> }).parameters
        ?.includeHighNibble,
    ).toBe(0);
  });
