...
const evidenceFor = (controller: ApplicationRuntime, claimId: string) =>
...
describe("timing evidence integrity", () => {
  it("verifies stability only from a valid attempt that reached the 15s window", async () => {
    ...
    expect(
      evidenceFor(controller, "graffiti.playback-stability")[0]?.status,
    ).toBe("verified");
  }, 30000);
  ...
  it("keeps invalid attempts in the report while excluding them from the conclusion", async () => {
    ...
    const report = store.controller.testReportMarkdown(
      "coolledux-graffiti-timing",
    );
    ...
    expect(report).toContain(
      "the measurement failed, not that the display behaved differently",
    );
  }, 30000);
});
    ...
    for (const step of store.getSnapshot().guidedFlow!.steps) {
      if (step.spec.kind === "choice")
        store.setGuidedObservation({
          kind: "choice",
          fieldId: step.spec.id,
          optionId: step.spec.id === "patch-0x0000" ? "off" : "red",
        });
    }
    store.submitGuidedObservations();
    const report = store.controller.testReportMarkdown(
