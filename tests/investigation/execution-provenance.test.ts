...
import { saveInvestigation, loadInvestigationHistory, toHistoricalInvestigation } from "../../src/storage/investigations";
import type { KeyValueStorage } from "../../src/storage/repository";
import type { DeviceFingerprint } from "../../src/core/device";
import { knownIledHatFingerprint } from "../helpers/fixtures";
import { uncharacterizedController, uncharacterizedRegistry } from "../helpers/uncharacterized-device";
...
async function deviceWith(fingerprint: DeviceFingerprint): Promise<MatrixController> {
...
describe("results link to the run that produced them", () => {
  it("stamps every new result with a real experiment run", async () => {
    ...
    const completed = controller.recordGuidedTestObservations("coolledux-graffiti-timing", baselineMoved, []);
    expect(completed.experimentRunId).toBeTruthy();
    const run = controller.experiments.find((entry) => entry.experimentRunId === completed.experimentRunId);
  ...
  it("ties repeated measurements of one experiment to the same run", async () => {
    ...
    const short = controller.recordGuidedTestObservations("coolledux-graffiti-timing", [
      { kind: "boolean", fieldId: "initial-correct", value: "yes" },
      timer("image-visible", 1200),
    ...
    const again = controller.recordGuidedTestObservations("coolledux-graffiti-timing", baselineMoved, []);
    expect(again.experimentRunId).toBe(short.experimentRunId);
    expect(controller.experiments.filter((run) => run.definitionId === "coolledux-graffiti-timing")).toHaveLength(1);
  }, 30000);

  it("gives an explicit reopen a new run", async () => {
    const { controller } = await uncharacterizedController();
    const first = controller.recordGuidedTestObservations("coolledux-graffiti-timing", baselineMoved, []);
    controller.reopenExperiment("coolledux-graffiti-timing", "double-checking the onset");
    const second = controller.recordGuidedTestObservations("coolledux-graffiti-timing", baselineMoved, []);
    expect(second.experimentRunId).not.toBe(first.experimentRunId);
    expect(controller.experiments.filter((run) => run.definitionId === "coolledux-graffiti-timing")).toHaveLength(2);
  }, 30000);
});
...
describe("historical results do not count as current-device execution", () => {
  it("still requires device B to run its own baseline", async () => {
    ...
    deviceA.recordGuidedTestObservations("coolledux-graffiti-timing", baselineMoved, []);
    // Device A may now run the stayTime variant, which requires the baseline.
    expect(deviceA.guidedTests().find((entry) => entry.test.id === "coolledux-graffiti-staytime")?.available).toBe(true);
    saveInvestigation(deviceA.investigation!, storage);

    const deviceB = await deviceWith(secondPhysicalDevice());
    deviceB.adoptInvestigation(toHistoricalInvestigation(loadInvestigationHistory(storage)[0]!.investigation));
    ...
    const staytime = deviceB.guidedTests().find((entry) => entry.test.id === "coolledux-graffiti-staytime")!;
    expect(staytime.available).toBe(false);
    expect(staytime.missingCompletedTests).toEqual(["coolledux-graffiti-timing"]);
  }, 30000);
  ...
  it("does not let a historical completion suppress the current device's experiment", async () => {
    ...
    const deviceA = await deviceWith(knownIledHatFingerprint());
    deviceA.recordGuidedTestObservations("coolledux-graffiti-timing", baselineMoved, []);
    expect(deviceA.recommendations().some((entry) => entry.testId === "coolledux-graffiti-timing")).toBe(false);
    saveInvestigation(deviceA.investigation!, storage);
    ...
    expect(deviceB.recommendations()[0]?.testId).toBe("coolledux-graffiti-timing");
    expect(deviceB.corePlanProgress()!.current?.step.testIds).toContain("coolledux-graffiti-timing");
  }, 30000);
  ...
  it("preserves execution continuity when the same authorized device resumes", async () => {
    ...
    resumed.adoptInvestigation(toHistoricalInvestigation(loadInvestigationHistory(storage)[0]!.investigation));
    // Same physical display: the execution record legitimately continues.
    expect(resumed.executedTests().map((test) => test.testId)).toEqual(["coolledux-graffiti-timing"]);
    expect(resumed.guidedTests().find((entry) => entry.test.id === "coolledux-graffiti-staytime")?.available).toBe(true);
  }, 30000);
});
