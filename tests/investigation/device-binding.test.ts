...
} from "../../src/investigation/device-identity";
...
function recordBlackPass(controller: ApplicationRuntime): void {
...
describe("cross-device evidence isolation", () => {
  it("stamps the investigation with the physical device binding", async () => {
    ...
    expect(controller.investigation?.deviceBinding?.browserDeviceId).toBe(
      "fixture-device",
    );
  });
  ...
  it("detaches the investigation when a different physical device connects", async () => {
    ...
    expect(
      controller
        .claims()
        .find((claim) => claim.id === "graffiti.black-semantics")?.decidedBy
        ?.scope,
    ...
    expect(
      state?.evidence.every((entry) => entry.scope !== "current-session"),
    ).toBe(true);
  ...
  it("keeps Device A evidence out of Device B recommendations and gates", async () => {
    ...
    transport.fingerprint = secondIledHatFingerprint();
    await controller.connect();
    const staticState = controller
      .claims()
      .find((claim) => claim.id === "animation.static-single-frame");
    ...
    expect(
      staticState?.evidence.some((entry) => entry.scope === "current-session"),
    ).toBe(false);
