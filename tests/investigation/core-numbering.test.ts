...
import {
  ...
  evaluateCorePlan,
  type CorePlan,
} from "../../src/investigation/core-plan";
...
const PLAN: CorePlan = {
  ...
  steps: [1, 2, 3, 4, 5, 6].map((ordinal) => ({
    ...
    title: `Step ${ordinal}`,
    purpose: "test",
    testIds: [`test-${ordinal}`],
    satisfiedBy: [],
    ...(ordinal === 5
      ? { skipWhen: () => "the branch that needed it was closed" }
      : {}),
  })),
};

const completed = (ids: readonly string[]) =>
  ids.map((testId) => ({
    testId,
    title: testId,
    startedAt: "",
...
describe("the denominator never moves", () => {
  ...
  it("reports completion as completed + skipped", () => {
    const progress = evaluateCorePlan(
      PLAN,
      NO_EVIDENCE,
      completed(["test-1", "test-2", "test-3", "test-4", "test-6"]),
    );
  ...
  it("is not complete while an unskipped slot is outstanding", () => {
    const progress = evaluateCorePlan(
      PLAN,
      NO_EVIDENCE,
      completed(["test-1", "test-2", "test-3", "test-4"]),
    );
  ...
  it("holds the total steady as a branch closes mid-investigation", () => {
    const neverSkips: CorePlan = {
      ...PLAN,
      steps: PLAN.steps.map((step) => ({ ...step, skipWhen: undefined })),
    };
    ...
    expect(displayPosition(before, "step-6")).toBe(
      displayPosition(after, "step-6"),
    );
  });
});
...
describe("the shipped iLedHat plan", () => {
  it("has six stable slots numbered 1 to 6", () => {
    expect(ILEDHAT_CORE_PLAN.steps.map((step) => step.ordinal)).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
  ...
  it("keeps the support decision at position 6 once the fallback is skipped", () => {
    ...
    const evidence: ClaimEvidence[] = [
      ...
      },
      {
        ...
        provenance: "observed",
        summary: "holds still",
        metrics: { visibleStaticHoldMs: 20000 },
      },
      {
    ...
    expect(
      progress.steps.find((entry) => entry.step.id === "pixel-mapping")!.step
        .ordinal,
    ).toBe(5);
    expect(progress.total).toBe(6);
