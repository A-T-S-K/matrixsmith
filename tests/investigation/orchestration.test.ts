...
describe("core plan progress", () => {
  it("numbers the experiment by the milestone it is currently serving", async () => {
    ...
    expect(flow(store).corePosition).toMatchObject({
  ...
  it("advances to the next distinct milestone after a conclusion", async () => {
    ...
    expect(store.getSnapshot().coreProgress).toMatchObject({
      completed: 1,
      total: 6,
    });
  ...
  it("keeps the denominator stable when a milestone is skipped", async () => {
    ...
    expect(before.steps.map((step) => step.position)).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
  }, 30000);
});
...
describe("retries stay one numbered test", () => {
  ...
  it("records each retry as the same experiment with a distinct attempt and reason", async () => {
    ...
    store.markObservationMissed("missed-t1");
    await store.retryTimingAttempt();
    const experiments = store.controller.experiments.filter(
    ...
    const reasons = store.controller.transfers.map(
  ...
  it("resends the identical experiment on retry — same parameters, same fingerprint", async () => {
    ...
    expect(second!.fingerprint.programCrc32).toBe(
      first!.fingerprint.programCrc32,
    );
  }, 30000);
  ...
  it("preserves invalid attempts without letting them establish anything", async () => {
    ...
    const run = store.controller.experiments.find(
      (entry) => entry.definitionId === "coolledux-graffiti-timing",
    )!;
    expect(
      run.attempts.filter((attempt) => attempt.validity === "invalid"),
    ).toHaveLength(1);
  }, 30000);
});
...
describe("transmission safety", () => {
  ...
  it("blocks a resend that claims to be a new experiment when the diagnostic is already showing", async () => {
    ...
    await expect(
      store.controller.runGuidedTestTransfer("coolledux-pixel-channels", {
...
describe("transfer classification", () => {
  const fingerprint = (
    ...
    buildExecutionFingerprint({
      physicalDeviceKey: "browser-device-a",
      testId,
      diagnosticId: "d",
      parameters,
      programCrc32: crc,
  ...
  it("does not flag legitimate retries merely because the bytes match", () => {
    ...
    expect(summary.repeatedExecutions[0]).toMatchObject({
      transfers: 3,
    ...
    const summary = classifyTransfers([
      {
        ...
        diagnosticId: "d",
        reason: "initial-experiment",
        fingerprint: fingerprint("timing", { stayTime: 3 }, "0xAAAA"),
        transactionIds: [],
        startedAt: "",
        finalWriteAcceptedAt: null,
        failureReason: null,
      },
      {
        ...
        attemptId: "a2",
        diagnosticId: "d",
        reason: "controlled-variant",
        fingerprint: fingerprint("staytime", { stayTime: 0 }, "0xBBBB"),
        transactionIds: [],
        startedAt: "",
        finalWriteAcceptedAt: null,
        failureReason: null,
