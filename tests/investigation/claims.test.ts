...
describe("atomic claims", () => {
    ...
    const evidence: ClaimEvidence[] = [
      {
        ...
        summary: "upstream white sentinel",
      },
      {
        claimId: "graffiti.black-semantics",
        status: "verified",
        scope: "current-session",
        provenance: "observed",
        summary: "0x0000 rendered off on this panel",
      },
    ];
  ...
  it("prefers a rejection over a verification within the same scope", () => {
    const evidence: ClaimEvidence[] = [
      sessionVerified("graffiti.playback-stability"),
      {
        ...
        scope: "current-session",
        provenance: "observed",
        summary: "raster moved after 3.2 seconds",
      },
    ];
    expect(claimState("graffiti.playback-stability", evidence).status).toBe(
      "rejected",
    );
  });
  ...
  it("does not let a partial observation verify a broad capability", () => {
    ...
    expect(claimState("graffiti.initial-render", evidence).status).toBe(
      "verified",
    );
    expect(claimState("graffiti.playback-stability", evidence).status).toBe(
      "unknown",
    );
    expect(claimState("graffiti.black-semantics", evidence).status).toBe(
      "unknown",
    );
  ...
  it("keeps autonomous looping distinct from power-cycle persistence", () => {
    ...
    expect(claimState("animation.autonomous-loop", evidence).status).toBe(
      "verified",
    );
    expect(claimState("power-cycle.persistence", evidence).status).toBe(
      "unknown",
    );
  });
  ...
  it("keeps stored-program upload separate from the content semantics inside it", () => {
    ...
    expect(claimState("stored-program.upload", evidence).status).toBe(
      "verified",
    );
    expect(claimState("graffiti.initial-render", evidence).status).toBe(
      "unknown",
...
describe("coolLedUxBaselineClaimEvidence", () => {
  ...
  it("reflects the physically characterized iLedHat evidence honestly", () => {
    ...
    expect(claimState("animation.black-semantics", evidence).status).toBe(
      "verified",
    );
    expect(claimState("graffiti.initial-render", evidence).status).toBe(
      "verified",
    ...
    expect(claimState("pixel.fourth-channel", evidence).status).toBe(
      "rejected",
    );
    expect(claimState("pixel.white-channel", evidence).status).toBe("rejected");
    expect(claimState("animation.static-single-frame", evidence).status).toBe(
      "verified",
    );
    expect(claimState("animation.static-identical-pair", evidence).status).toBe(
      "verified",
    );
    expect(claimState("pixel.channel-map", evidence).status).toBe("verified");
    expect(claimState("pixel.encoder-correctness", evidence).status).toBe(
      "verified",
    );
    // Still genuinely open, and deliberately so.
    expect(claimState("pixel.color-calibration", evidence).status).toBe(
      "unresolved",
    );
    expect(claimState("power-cycle.persistence", evidence).status).toBe(
      "unknown",
  ...
  it("never marks physically untested claims as current-session", () => {
    for (const entry of evidence)
      expect(entry.scope).not.toBe("current-session");
  });

  it("only ships source-reference evidence for unknown profiles", () => {
    const generic = coolLedUxBaselineClaimEvidence({
      ...iledHat31aeProfile,
