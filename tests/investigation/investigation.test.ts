...
function makeTest(
  overrides: Partial<CompletedGuidedTest> = {},
): CompletedGuidedTest {
  return {
    ...
    established: ["Graffiti 0x0000 renders off/black on this iLedHat."],
    rejected: [],
    unknowns: [],
    summary: "0x0000 columns were off.",
...
describe("Investigation", () => {
  it("creates an active investigation with a goal", () => {
    const investigation = createInvestigation({
      profileId: iledHat31aeProfile.id,
      deviceName: "iLedHat",
      goal: { kind: "develop", description: "Characterize static rendering" },
    });
  ...
  it("records completed tests with their claim evidence", () => {
    ...
    expect(hasCompletedTest(investigation, "coolledux-graffiti-black")).toBe(
      true,
    );
    expect(
      latestTestResult(investigation, "coolledux-graffiti-black")?.status,
    ).toBe("passed");
    const claims = investigationClaims(investigation);
    expect(
      claims.find((claim) => claim.id === "graffiti.black-semantics")?.status,
    ).toBe("verified");
  });
  ...
  it("combines session evidence with baseline evidence, current session winning", () => {
      ...
      [
        {
          ...
          scope: "current-session",
          provenance: "observed",
          summary: "0x0000 rendered bright white here too",
          testId: "coolledux-graffiti-black",
        },
      ],
    );
    ...
    expect(
      claims.find((claim) => claim.id === "brightness.control")?.status,
    ).toBe("verified");
  });
  ...
  it("keeps failed tests as useful evidence rather than discarding them", () => {
    ...
    const failed = makeTest({
      ...
      established: [
        "Upload worked.",
        "Full raster appeared.",
        "Tiles aligned.",
      ],
      rejected: [
        "Image did not remain static; movement began after 3.2 seconds.",
      ],
    });
    investigation = recordCompletedTest(investigation, failed, [
      ...
      },
      {
        ...
        scope: "current-session",
        provenance: "observed",
        summary: "movement after 3.2 s",
        testId: failed.testId,
      },
    ]);
    const claims = investigationClaims(investigation);
    expect(
      claims.find((claim) => claim.id === "graffiti.initial-render")?.status,
    ).toBe("verified");
    expect(
      claims.find((claim) => claim.id === "graffiti.playback-stability")
        ?.status,
    ).toBe("rejected");
    expect(
      latestTestResult(investigation, failed.testId)?.rejected,
    ).toHaveLength(1);
  });
  ...
  it("supports stop and resume without losing evidence", () => {
    let investigation = createInvestigation({
      ...
      goal: {
        ...
        description: SYMPTOM_LABELS["content-moves-unexpectedly"],
      },
    });
    investigation = recordCompletedTest(investigation, makeTest(), []);
  ...
  it("maps every symptom to labels and focus claims", () => {
    for (const symptomId of Object.keys(
      SYMPTOM_LABELS,
    ) as (keyof typeof SYMPTOM_LABELS)[]) {
  ...
  it("round-trips through JSON serialization", () => {
    let investigation = createInvestigation({
      profileId: iledHat31aeProfile.id,
      deviceName: "iLedHat",
      goal: { kind: "develop", description: "x" },
    });
    investigation = recordCompletedTest(
      investigation,
      makeTest({
    ...
    expect(claimState("brightness.control", parsed.claimEvidence).status).toBe(
      "unknown",
    );
  });
});
