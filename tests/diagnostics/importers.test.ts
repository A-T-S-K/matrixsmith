...
import { EVIDENCE_IMPORTERS, findImporter, nrfConnectTextLogImporter } from "../../src/diagnostics/importers";
...
import { generateMarkdownReport, type ReportData } from "../../src/diagnostics/report";
...
const FIXTURE = readFileSync(new URL("../fixtures/nrf/iledhat-session.txt", import.meta.url), "utf8");
...
describe("nRF Connect text log importer", () => {
  ...
  it("extracts the device name and keeps the BLE address out of the fingerprint", () => {
    ...
    expect(JSON.stringify(result.fingerprint)).not.toContain("01:00:00:21:CC:99");
  });

  it("extracts the GATT hierarchy with normalized UUIDs and properties", () => {
    const result = parseFixture();
    const fff0 = result.fingerprint?.services.find((service) => service.uuid === "0000fff0-0000-1000-8000-00805f9b34fb");
    expect(fff0).toBeDefined();
    const fff1 = fff0?.characteristics.find((c) => c.uuid === "0000fff1-0000-1000-8000-00805f9b34fb");
    expect(fff1?.properties).toEqual({ read: true, write: false, writeWithoutResponse: true, notify: true, indicate: false });
    const genericAccess = result.fingerprint?.services.find((service) => service.uuid === "00001800-0000-1000-8000-00805f9b34fb");
    expect(genericAccess?.characteristics.some((c) => c.uuid === "00002a00-0000-1000-8000-00805f9b34fb")).toBe(true);
  });
  ...
  it("extracts TX writes and RX notifications with exact bytes and timestamps", () => {
    const result = parseFixture();
    const deviceInfo = result.transactions.find((t) => t.operation === "GetDeviceInfo (imported)");
    expect(deviceInfo).toBeDefined();
    expect(deviceInfo?.packets.find((p) => p.direction === "TX")?.hex).toBe(dash(infoCc.txHex));
    expect(deviceInfo?.packets.find((p) => p.direction === "RX")?.hex).toBe(dash(infoCc.rxHex));
  ...
  it("correlates TX and RX by endpoint, timing, and envelope opcode", () => {
    const result = parseFixture();
    const brightness = result.transactions.find((t) => t.operation === "SetBrightness (imported)");
  ...
  it("decodes imported packets with installed drivers", () => {
    const result = parseFixture();
    const deviceInfos = result.transactions.filter((t) => t.decodedResponse?.kind === "device-info");
    ...
    const rejected = result.transactions.find((t) => t.packets.some((p) => p.hex === dash(rejection.rxHex)));
  ...
  it("classifies imported program uploads as persistent without inventing semantics", () => {
    ...
    const result = nrfConnectTextLogImporter.parse(log, { drivers: builtInDrivers });
    const announce = result.transactions.find((t) => t.operation === "Program announce (imported)");
  ...
  it("warns on malformed hex without aborting the import", () => {
    ...
    const result = nrfConnectTextLogImporter.parse(log, { drivers: builtInDrivers });
    expect(result.warnings.some((warning) => warning.includes("parseable bytes"))).toBe(true);
  ...
  it("returns an explicit warning for content with nothing recognizable", () => {
    const result = nrfConnectTextLogImporter.parse("just some text\nmore text", {});
  ...
  it("redacts BLE addresses in imported error details", () => {
    ...
    expect(result.gattEvents.find((event) => event.kind === "error")?.detail).not.toContain("AA:BB:CC:DD:EE:FF");
  });

  it("preserves the raw log only when requested", () => {
    expect(nrfConnectTextLogImporter.parse(FIXTURE, {}).rawLog).toBeUndefined();
    expect(nrfConnectTextLogImporter.parse(FIXTURE, { keepRawLog: true }).rawLog).toBe(FIXTURE);
  });
});
...
describe("controller external import", () => {
  it("opens an offline imported session that cannot transmit", async () => {
    ...
    expect(controller.transactions.some((t) => t.sessionSource === "imported")).toBe(true);
    expect(controller.importedEvidence).toHaveLength(1);
    // Imported sessions must never transmit.
    await expect(controller.probe()).rejects.toThrow(/cannot perform live probes/);
  });
  ...
  it("merges imported evidence into an active live session without clearing it", async () => {
    ...
    expect(controller.transactions.some((t) => t.sessionSource === "imported")).toBe(true);
    expect(controller.transactions.some((t) => t.sessionSource === "live")).toBe(true);
  });
  ...
  it("labels imported transactions distinctly in the Markdown report", () => {
    ...
    const data: ReportData = {
      createdAt: "2026-08-31T21:00:00.000Z", matrixsmithVersion: "0.1.0",
      fingerprint: controller.session.fingerprint!, profile: controller.session.profile,
      selectedDriver: controller.session.selection?.selected?.id ?? null,
      driverMatches: controller.session.selection?.matches ?? [], capabilities: [],
      transactions: controller.transactions, diagnosticRuns: [], observations: controller.observations,
      trace: [], protocolResolution: null, validations: [], contentCompilations: [],
      importedEvidence: controller.importedEvidence, liveConnected: false, source: "imported",
    };
