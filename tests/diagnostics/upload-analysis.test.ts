...
import { analyzeStoredProgramUpload, describeUploadAnalysis } from "../../src/diagnostics/upload-analysis";
...
import type { ProtocolTransaction, TransactionPacket } from "../../src/diagnostics/transactions";

const ENDPOINT = { serviceUuid: "0000fff0-0000-1000-8000-00805f9b34fb", characteristicUuid: "0000fff1-0000-1000-8000-00805f9b34fb" };

function announceReceiptHex(statusRaw = 0x00): string { return packetHex(encodeEnvelope(Uint8Array.of(0x02, statusRaw))); }
function chunkReceiptHex(index: number, statusRaw = 0x00): string { return packetHex(encodeEnvelope(Uint8Array.of(0x03, 0x00, (index >> 8) & 0xff, index & 0xff, statusRaw))); }

function txPacket(index: number, baseMs: number, gapMs = 60): TransactionPacket {
  const at = baseMs + index * gapMs;
  return {
    timestamp: new Date(at).toISOString(), direction: "TX", hex: "01 00 02 05 1F 03", endpoint: ENDPOINT,
    hostAcceptedAt: new Date(at + 5).toISOString(), scheduledDelayMs: 60,
    ...(index > 0 ? { gapSincePreviousTxMs: gapMs } : {}),
  };
}

function rxPacket(hex: string, atMs: number): TransactionPacket {
  return { timestamp: new Date(atMs).toISOString(), direction: "RX", hex, endpoint: ENDPOINT };
}

function makeTransaction(packets: readonly TransactionPacket[]): ProtocolTransaction {
  return {
    id: "transaction:test", startedAt: packets[0]?.timestamp ?? new Date(0).toISOString(), completedAt: new Date(10_000).toISOString(),
    durationMs: 1000, sessionSource: "live", source: "operation", driverId: "coolledux", profileId: "iledhat-31ae-32x16",
    operation: "ShowFrame", safety: { risk: "persistent", persistence: "persistent", validation: "experimental" },
    endpoint: ENDPOINT, packets, decodedResponse: null, hostAccepted: true, protocolAcknowledged: null,
    deviceStateVerified: false, responseTimedOut: false, error: null, findings: [], observationIds: [], diagnosticRunId: null,
  };
}
...
describe("CoolLEDUX stored-program receipt decoding", () => {
  it("decodes the announce-style receipt structurally without success semantics", () => {
    const decoded = decodeCoolLedUxNotification(encodeEnvelope(Uint8Array.of(0x02, 0x00)));
  ...
  it("decodes chunk-style receipts with a structural chunk index and raw status", () => {
    const decoded = decodeCoolLedUxNotification(encodeEnvelope(Uint8Array.of(0x03, 0x00, 0x00, 0x02, 0x00)));
    expect(decoded?.kind).toBe("program-chunk-receipt");
    expect(decoded?.fields.chunkIndex).toBe(2);
    expect(decoded?.fields.statusRaw).toBe(0);
    expect(decoded?.summary).toContain("not a reliable per-chunk acknowledgement");
  });

  it("decodes the physically observed announce receipt bytes 01 00 02 06 02 06 00 03", () => {
    const decoded = decodeCoolLedUxNotification(Uint8Array.of(0x01, 0x00, 0x02, 0x06, 0x02, 0x06, 0x00, 0x03));
    expect(decoded?.kind).toBe("program-announce-receipt");
    expect(decoded?.payloadHex).toBe("02 00");
  });

  it("still decodes brightness echoes and device info normally", () => {
    const brightness = decodeCoolLedUxNotification(encodeEnvelope(Uint8Array.of(0x04, 0x40)));
    expect(brightness?.kind).toBe("command-echo");
    const info = decodeCoolLedUxNotification(encodeEnvelope(Uint8Array.of(0x1f, 0x01, 0xcc)));
...
describe("analyzeStoredProgramUpload", () => {
  ...
  it("correlates observed chunk receipts and reports missing ones as observations, not failures", () => {
    ...
    const analysis = analyzeStoredProgramUpload(makeTransaction(packets), 4, decodeCoolLedUxNotification);
  ...
  it("flags out-of-range chunk indices without inventing meaning", () => {
    const packets: TransactionPacket[] = [txPacket(0, base), rxPacket(chunkReceiptHex(9), base + 100)];
    const analysis = analyzeStoredProgramUpload(makeTransaction(packets), 2, decodeCoolLedUxNotification);
  ...
  it("summarizes measured timing from real per-packet timestamps", () => {
    ...
    const analysis = analyzeStoredProgramUpload(makeTransaction(packets), 3, decodeCoolLedUxNotification);
  ...
  it("keeps raw non-zero status bytes verbatim", () => {
    const packets: TransactionPacket[] = [txPacket(0, base), rxPacket(chunkReceiptHex(0, 0x7f), base + 90)];
    const analysis = analyzeStoredProgramUpload(makeTransaction(packets), 1, decodeCoolLedUxNotification);
