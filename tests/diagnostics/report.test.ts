...
function sampleData(): ReportData {
  ...
  return {
    ...
    driverMatches: [
      {
        ...
        score: 80,
        confidence: "strong",
        reasons: ["Structured 0x1F device info"],
        contradictions: [],
      },
      {
        driverId: "coolledx",
        ...
        reasons: [],
        contradictions: [
          "Classic brightness hypothesis returned 08 FE and no visible effect",
        ],
      },
    ],
    capabilities,
    ...
    transactions: [
      {
        ...
        completedAt: "2026-08-31T12:00:01.200Z",
        durationMs: 200,
        sessionSource: "live",
        source: "probe",
        driverId: "coolledux",
        ...
        packets: [
          ...
          {
            timestamp: "2026-08-31T12:00:01.150Z",
            direction: "RX",
            hex: infoCc.rxHex,
          },
        ],
        ...
        deviceStateVerified: true,
        responseTimedOut: false,
        error: null,
        findings: ["Structured 0x1F response"],
        observationIds: [],
    ...
    observations: [
      {
        ...
        summary: "Panel visibly dimmed.",
        confidence: "observed",
      },
    ],
    trace: [
      {
        timestamp: "2026-08-31T12:00:00.500Z",
        type: "app.started",
        metadata: {
    ...
    protocolResolution: {
      summary: "Valid structured 0x1F response",
      source: "live-probe",
    },
    contentCompilations: [],
    importedEvidence: [],
...
describe("markdown report generator", () => {
  it("is deterministic for identical inputs", () => {
    expect(generateMarkdownReport(sampleData())).toBe(
      generateMarkdownReport(sampleData()),
    );
  });
  ...
  it("excludes browser/device identifiers by default and includes them only on request", () => {
    ...
    const full = generateMarkdownReport(sampleData(), {
      ...DEFAULT_REPORT_OPTIONS,
  ...
  it("suggests next tests that advance the support state", () => {
    ...
    const ambiguous = generateMarkdownReport({
      ...data,
      selectedDriver: null,
    ...
    expect(ambiguous).toContain("Identify this display");
  });
