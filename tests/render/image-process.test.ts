...
describe("content-routed image processing", () => {
  ...
  it("keeps pixel art hard, deterministic, and RGB444-realizable", () => {
    const input = source(4, 4, (x, y) =>
      ((x >> 1) + (y >> 1)) % 2 ? [255, 128, 0, 255] : [0, 0, 0, 255],
    );
    const recipe = {
      mode: "pixel-art" as const,
      composition: { mode: "contain" as const },
    };
  ...
  it("routes uncertain Auto content to the Photo-safe path", () => {
    ...
    const result = processImage(
      ...
      32,
      16,
      { mode: "auto", composition: { mode: "contain" } },
      ILEDHAT_RGB444,
    );
