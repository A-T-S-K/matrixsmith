...
describe("supplied-logo production reducer golden invariants", () => {
  it("retains black, the orange accent, a bounded hard palette, determinism, and interactive runtime", () => {
    const source = decodeRgbaPng(
      readFileSync("research/image-reduction/fixtures/atsk-logo.png"),
    );
    const recipe = {
      mode: "artwork" as const,
      composition: { mode: "foreground-trim" as const, opticalScaleX: 1.35 },
    };
    const started = performance.now();
    const first = processImage(source, 32, 16, recipe, ILEDHAT_RGB444);
    const elapsed = performance.now() - started;
    const second = processImage(source, 32, 16, recipe, ILEDHAT_RGB444);
    expect([...first.frame.data]).toEqual([...second.frame.data]);
    expect(first.frame.width).toBe(32);
    expect(first.frame.height).toBe(16);
    expect(first.outputColorCount).toBeGreaterThanOrEqual(3);
    expect(first.outputColorCount).toBeLessThanOrEqual(6);
    ...
    for (let y = 0; y < 16; y += 1)
      for (let x = 0; x < 32; x += 1) {
        const pixel = first.frame.getPixel(x, y);
        if (pixel.r === 0 && pixel.g === 0 && pixel.b === 0) black += 1;
        if (pixel.r > pixel.g + 50 && pixel.r > pixel.b + 80) {
          orange += 1;
    ...
    expect(orange).toBeGreaterThan(4);
    expect((orangeMinX + orangeMaxX) / 2).toBeGreaterThan(12);
    expect((orangeMinX + orangeMaxX) / 2).toBeLessThan(20);
  ...
  it.each([
    ...
    const source = decodeRgbaPng(
      readFileSync(`research/image-reduction/fixtures/${name}`),
    );
...
function decodeRgbaPng(bytes: Uint8Array): DecodedImageSource {
  ...
  const raw = inflateSync(
    Buffer.concat(idat.map((value) => Buffer.from(value))),
  );
  const stride = width * channels;
  const rgba = new Uint8ClampedArray(width * height * channels);
  let input = 0;
  let transparent = 0;
  for (let y = 0; y < height; y += 1) {
    const filter = raw[input++] ?? 0;
    for (let x = 0; x < stride; x += 1) {
      const value = raw[input++] ?? 0;
      const left = x >= channels ? (rgba[y * stride + x - channels] ?? 0) : 0;
      const up = y > 0 ? (rgba[(y - 1) * stride + x] ?? 0) : 0;
      const upLeft =
        y > 0 && x >= channels
          ? (rgba[(y - 1) * stride + x - channels] ?? 0)
          : 0;
      rgba[y * stride + x] =
        (filter === 0
          ? value
  ...
  for (let i = 3; i < rgba.length; i += 4)
    if ((rgba[i] ?? 255) < 255) transparent += 1;
  return {
    rgba,
    width,
    height,
