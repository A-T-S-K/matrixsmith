...
import { DEFAULT_CONTENT_SETTINGS, loadContentSettings, saveContentSettings } from "../../src/storage/settings";
...
describe("content settings persistence", () => {
  it("round-trips settings and merges defaults for missing keys", () => {
    ...
    saveContentSettings({ ...DEFAULT_CONTENT_SETTINGS, text: "HELLO", lastBrightness: 0x40 }, storage);
  ...
  it("stores only small JSON, never binary media", () => {
    const storage = memoryStorage();
    saveContentSettings({ ...DEFAULT_CONTENT_SETTINGS, text: "HELLO" }, storage);
  ...
  it("falls back to defaults on corrupt or missing data", () => {
    ...
    storage.setItem("matrixsmith:v1:content-settings", "{not json");
    expect(loadContentSettings(storage)).toEqual(DEFAULT_CONTENT_SETTINGS);
    storage.setItem("matrixsmith:v1:content-settings", JSON.stringify({ schemaVersion: 99 }));
    expect(loadContentSettings(storage)).toEqual(DEFAULT_CONTENT_SETTINGS);
  });
});
