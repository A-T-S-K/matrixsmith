...
import {
  ...
  type ExperimentRun,
  type TransferRecord,
} from "../../src/investigation/orchestration";
...
const MISSED_ATTEMPT: ObservationAttempt = {
  ...
  values: [],
  validity: "missed-t1",
  invalidationReason: "The moment the full image appeared was missed",
  note: null,
  startedAt: "2026-09-01T10:00:00.000Z",
  endedAt: "2026-09-01T10:00:04.000Z",
};

const VALID_ATTEMPT: ObservationAttempt = {
  attemptNumber: 2,
  parameters: { stayTime: 3 },
  t0: "2026-09-01T10:01:00.000Z",
  marks: [
    {
      event: "visible",
      timestamp: "2026-09-01T10:01:00.400Z",
      source: "human-observed",
      elapsedMs: 400,
    ...
    {
      event: "movement",
      timestamp: "2026-09-01T10:01:03.600Z",
      source: "human-observed",
      elapsedMs: 3600,
  ...
  endedAt: "2026-09-01T10:01:03.600Z",
};
...
const TRANSFERS: TransferRecord[] = [
  {
    ...
    startedAt: "2026-09-01T10:00:00.000Z",
    finalWriteAcceptedAt: "2026-09-01T10:00:00.000Z",
    failureReason: null,
  },
  {
    ...
    attemptId: "attempt:2",
    diagnosticId: "graffiti-timing-probe",
    reason: "explicit-retry-missed-observation",
    fingerprint: FINGERPRINT,
    transactionIds: ["transaction:2"],
    startedAt: "2026-09-01T10:01:00.000Z",
    finalWriteAcceptedAt: "2026-09-01T10:01:00.000Z",
    failureReason: null,
  },
];
...
const RUN: ExperimentRun = {
  ...
  definitionId: "coolledux-graffiti-timing",
  title: "Measure how long a static image stays still",
  corePlanStepId: "playback-discriminator",
  variant: "stayTime=3",
  parameters: { stayTime: 3 },
  fingerprint: FINGERPRINT,
  status: "partial",
  resolution: "settled",
  startedAt: "2026-09-01T10:00:00.000Z",
  completedAt: "2026-09-01T10:01:04.000Z",
  attempts: [
    {
      attemptId: "attempt:1",
      attemptNumber: 1,
      ...
      observations: [],
      timing: MISSED_ATTEMPT,
      validity: "invalid",
      invalidationReason: "The moment the full image appeared was missed",
      failureKind: "human-missed",
    },
    {
      attemptId: "attempt:2",
      attemptNumber: 2,
      reason: "explicit-retry-missed-observation",
      startedAt: "2026-09-01T10:01:00.000Z",
      transferIds: ["transfer:2"],
      observations: VALID_ATTEMPT.values,
      timing: VALID_ATTEMPT,
      validity: "valid",
      invalidationReason: null,
      failureKind: null,
    },
  ],
  conclusion: "The image appeared correctly, then started moving.",
  reopenReason: null,
};
...
function investigationWithOrchestration(): Investigation {
  ...
  investigation = recordCompletedTest(
    investigation,
    {
      ...
      observations: VALID_ATTEMPT.values,
      established: [],
      rejected: ["It did not stay still."],
      unknowns: [],
      summary: "moved",
      transactionIds: ["transaction:1", "transaction:2"],
      parameters: { stayTime: 3 },
      attempts: [MISSED_ATTEMPT, VALID_ATTEMPT],
      resolution: "settled",
    },
    [],
  );
  return withOrchestration(investigation, {
    ...
    panelProgram: {
      ...
      fingerprint: FINGERPRINT,
      label: "guided diagnostic timing",
      startedAt: "2026-09-01T10:00:59.000Z",
      writtenAt: "2026-09-01T10:01:00.000Z",
    ...
    recommendationTrail: [
      {
        ...
        evidenceCount: 0,
        origin: "automatic-recommendation",
      },
    ],
  });
}
...
describe("orchestration survives local persistence", () => {
  it("round-trips experiments, attempts and transfers", () => {
    ...
    expect(run.fingerprint.deviceIdentityBasis).toBe(
  ...
  it("keeps every transfer reason and its transaction references", () => {
    ...
    const transfers =
      loadInvestigationHistory(storage)[0]!.investigation.orchestration
        .transfers;
    expect(transfers.map((transfer) => transfer.reason)).toEqual([
      "initial-experiment",
  ...
  it("keeps reopen state and the recommendation trail", () => {
    ...
    expect(orchestration.recommendationTrail[0]).toMatchObject({
      testId: "coolledux-graffiti-timing",
      origin: "automatic-recommendation",
    });
  });
  ...
  it("keeps the exact invalid/valid distinction on the completed test", () => {
    ...
    saveInvestigation(investigationWithOrchestration(), storage);
    const completed =
      loadInvestigationHistory(storage)[0]!.investigation.completedTests[0]!;
    ...
    expect(completed.attempts![1]!.marks.map((mark) => mark.event)).toEqual([
      "visible",
      "movement",
    ]);
...
describe("restored orchestration carries no operational authority", () => {
  ...
  it("ignores a stored record that claims a diagnostic is live", () => {
    ...
    storage.setItem("matrixsmith:v2:investigations", poisoned);
    const panel =
      loadInvestigationHistory(storage)[0]!.investigation.orchestration
        .panelProgram;
    expect(panel.certainty).toBe("unknown");
...
describe("schema migration", () => {
  it("does not migrate v1 records", () => {
    const storage = memoryStorage();
    ...
    storage.setItem("matrixsmith:v2:investigations", v1);
    expect(loadInvestigationHistory(storage)).toEqual([]);
  });

  it("writes only the current schema version", () => {
    const storage = memoryStorage();
    saveInvestigation(investigationWithOrchestration(), storage);
    const raw = JSON.parse(
      storage.getItem("matrixsmith:v2:investigations")!,
  ...
  it("drops a record from an unknown future schema rather than guessing", () => {
    ...
    storage.setItem(
      ...
      JSON.stringify({
        schemaVersion: 99,
        investigations: [{ savedAt: "x", investigation: {} }],
      }),
    );
    expect(loadInvestigationHistory(storage)).toEqual([]);
  });
  ...
  it("rejects structurally invalid orchestration entries without losing the rest", () => {
    ...
    storage.setItem("matrixsmith:v2:investigations", corrupt);
    const orchestration =
      loadInvestigationHistory(storage)[0]!.investigation.orchestration;
...
describe("forgetting local history removes orchestration too", () => {
  it("leaves nothing behind", () => {
    ...
    expect(storage.getItem("matrixsmith:v2:investigations")).toBeNull();
  });
});
