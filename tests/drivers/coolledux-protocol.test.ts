...
import { encodeBrightness, encodeDeviceInfoQuery } from "../../src/drivers/coolledux/protocol";
...
describe("CoolLEDUX direct protocol", () => {
  ...
  it.each([[infoCc.rxHex, 0xcc], [info40.rxHex, 0x40]])("decodes the 48-byte hardware device-info response", (raw, brightness) => {
    ...
    expect(decoded?.fields).toMatchObject({ payloadLength: 48, powerRaw: 1, powerOn: true, brightnessRaw: brightness });
  ...
  it("accepts a variable-length known prefix and rejects a truncated one conservatively", () => {
    expect(decodeCoolLedUxNotification(parseHexBytes("0100041F0205405503"))?.kind).toBe("device-info");
    expect(decodeCoolLedUxNotification(parseHexBytes("010002061F020503"))?.kind).toBe("malformed-device-info");
  });
  it("decodes a brightness echo", () => expect(decodeCoolLedUxNotification(parseHexBytes("01000206044003"))?.fields.brightnessRaw).toBe(0x40));
  it("preserves classic 08 FE as rejection-like with unmapped semantics", () => {
    const decoded = decodeCoolLedNotification(parseHexBytes(classicRejection.rxHex));
    expect(decoded).toMatchObject({ family: "CoolLEDX", opcode: 8, status: 0xfe, success: false });
