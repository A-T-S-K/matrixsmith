...
import {
  COOLLEDUX_DIAGNOSTIC_CONTENT,
  diagnosticContent,
  PIXEL_CHANNEL_PROBE_WORDS,
} from "../../src/drivers/coolledux/diagnostics";
...
function programBytes(compiledProgram: {
...
function collectWords(program: Uint8Array): Set<number> {
  ...
  for (let index = 0; index + 1 < program.length; index += 2)
    words.add(((program[index] ?? 0) << 8) | (program[index + 1] ?? 0));
  return words;
}
...
describe("graffiti black/off probe", () => {
  ...
  it("preserves literal raw 0x0000 and 0x0004 with no substitution", () => {
    const zeroRegion = built.regions.find(
      (region) => region.technical.rawWord === 0x0000 && region.width === 8,
    );
    const workaroundRegion = built.regions.find(
      (region) => region.technical.rawWord === 0x0004,
    );
  ...
  it("uses alternating 8-column regions across the canvas", () => {
    ...
    expect(bands.map((region) => region.technical.rawWord)).toEqual([
...
describe("graffiti timing probe / stayTime discriminator", () => {
  it("keeps every byte identical between stayTime variants except the stayTime byte", () => {
    const baseline = diagnosticContent("graffiti-timing-probe").build(PROFILE, {
      stayTime: 3,
    });
    const variant = diagnosticContent("graffiti-timing-probe").build(PROFILE, {
      stayTime: 0,
    });
    ...
    const diffs: number[] = [];
    for (let index = 0; index < baseBytes.length; index += 1)
      if (baseBytes[index] !== variantBytes[index]) diffs.push(index);
    ...
    for (const index of diffs) {
      expect(baseBytes[index]).toBe(3);
      expect(variantBytes[index]).toBe(0);
    }
  });

  it("refuses stayTime values without credible source evidence", () => {
    expect(() =>
      diagnosticContent("graffiti-timing-probe").build(PROFILE, {
        stayTime: 0xff,
      }),
    ).toThrow(/allowed/i);
    expect(() =>
  ...
  it("records the tested playback parameters", () => {
    const built = diagnosticContent("graffiti-timing-probe").build(PROFILE, {
      stayTime: 0,
    });
...
describe("animation static raster", () => {
  ...
  it("offers the identical-two-frame variant as a separate explicit build", () => {
    const single = diagnosticContent("animation-static-raster").build(PROFILE, {
      frames: 1,
    });
    const pair = diagnosticContent("animation-static-raster").build(PROFILE, {
      frames: 2,
    });
    expect(pair.frameCount).toBe(2);
    expect(pair.compiled.programBytes.length).toBeGreaterThan(
      single.compiled.programBytes.length,
    );
    expect(() =>
      diagnosticContent("animation-static-raster").build(PROFILE, {
        frames: 3,
      }),
    ).toThrow(/allowed/i);
  });
...
describe("pixel-channel probe", () => {
  ...
  it("transmits every declared raw word exactly, including untransformed high nibbles", () => {
    ...
    for (const word of PIXEL_CHANNEL_PROBE_WORDS)
  ...
  it("labels only RGB444-predictable words with an expectation", () => {
    const expectations = built.regions.filter(
      (region) => region.technical.expectedUnderHypothesis,
    );
    expect(
      expectations
        .map((region) => region.technical.rawWord!)
        .sort((a, b) => a - b),
    ).toEqual([0x0000, 0x000f, 0x00f0, 0x0f00, 0x0fff, 0xffff]);
    const highNibbleOnly = built.regions.filter((region) =>
      [0x1000, 0x2000, 0x4000, 0x8000, 0xf000].includes(
        region.technical.rawWord ?? -1,
      ),
    );
    for (const region of highNibbleOnly)
      expect(region.technical.expectedUnderHypothesis).toBeUndefined();
  });

  it("keeps the lit area conservative", () => {
    const litPixels = built.regions.reduce(
...
describe("no general raw writer", () => {
  it("only accepts declared diagnostic ids through ShowDiagnostic", () => {
    expect(() =>
      coolLedUxDriver.plan(
        { type: "ShowDiagnostic", diagnosticId: "arbitrary-bytes" },
        context,
      ),
    ).toThrow(/Unknown CoolLEDUX diagnostic/);
  });
  ...
  it("plans diagnostics as persistent content with honest metadata", () => {
    const plan = coolLedUxDriver.plan(
      { type: "ShowDiagnostic", diagnosticId: "graffiti-black-probe" },
      context,
    );
