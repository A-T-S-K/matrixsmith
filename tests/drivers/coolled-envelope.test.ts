...
import {
  ...
  escapeBytes,
  unescapeBytes,
} from "../../src/drivers/coolled/common/envelope";
...
describe("shared CoolLED envelope", () => {
  ...
  it("escapes and unescapes every reserved byte", () => {
    expect([...escapeBytes(Uint8Array.of(1, 2, 3))]).toEqual([
      2, 5, 2, 6, 2, 7,
    ]);
    expect([...unescapeBytes(Uint8Array.of(2, 5, 2, 6, 2, 7))]).toEqual([
      1, 2, 3,
    ]);
  });
  it("rejects malformed framing, escapes, and length", () => {
    ...
    expect(() => decodeEnvelope(parseHexBytes("0100000203"))).toThrow(
      /truncated escape/,
    );
    expect(() => decodeEnvelope(parseHexBytes("010002064003"))).toThrow(
      /length mismatch/,
    );
  });
  it.each([
    ...
    ["01000206044003", "0440"],
    ["010002051F03", "1F"],
  ])("decodes hardware packet %s", (raw, payload) =>
    expect(compactHex(decodeEnvelope(parseHexBytes(raw)).payload)).toBe(
      payload,
    ),
  );
});
