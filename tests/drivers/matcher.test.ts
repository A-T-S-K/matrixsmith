...
import { emptyFingerprint, type DeviceFingerprint } from "../../src/core/device";
...
describe("known-profile driver matching", () => {
  it("selects CoolLEDUX for the characterized iLedHat without any probe", () => {
    ...
    const coolledux = selection.matches.find(({ driverId }) => driverId === "coolledux")!;
  ...
  it("records the physical reason CoolLEDX is ruled out for that profile", () => {
    const selection = new DriverRegistry(builtInDrivers).match(knownIledHatFingerprint());
    const coolledx = selection.matches.find(({ driverId }) => driverId === "coolledx")!;
    expect(coolledx.score).toBe(0);
    expect(coolledx.confidence).toBe("none");
    expect(coolledx.contradictions.join(" ")).toMatch(/0x08 returned 0x08 0xFE/);
  });
  ...
  it("recognizes the profile without live manufacturer data", () => {
    ...
    delete (fingerprint as { manufacturerDataHex?: string }).manufacturerDataHex;
    delete (fingerprint as { rawAdvertisementHex?: string }).rawAdvertisementHex;
...
describe("unknown devices keep the conservative treatment", () => {
  it("stays ambiguous for the same GATT on an unrecognized display", () => {
    const selection = new DriverRegistry(builtInDrivers).match(unknownSharedTransport());
  ...
  it("withdraws recognition when the confirmed geometry contradicts the profile", () => {
    const fingerprint: DeviceFingerprint = { ...knownIledHatFingerprint(), manuallyConfirmedGeometry: { width: 64, height: 16 } };
    const selection = new DriverRegistry(builtInDrivers).match(fingerprint);
    expect(selection.selected).toBeNull();
    expect(coolLedUxDriver.resolveProfile(fingerprint)).toBeNull();
    expect(selection.matches.flatMap(({ contradictions }) => contradictions).join(" ")).toMatch(/64×16 geometry contradicts/);
  });

  it("withdraws recognition when the manufacturer field contradicts the profile", () => {
    const fingerprint: DeviceFingerprint = { ...knownIledHatFingerprint(), manufacturerDataHex: "FFFF5EEA07000001100020031E" };
    const selection = new DriverRegistry(builtInDrivers).match(fingerprint);
    expect(selection.selected).toBeNull();
    expect(coolLedUxDriver.resolveProfile(fingerprint)).toBeNull();
  });
  ...
  it("withdraws recognition when the FFF1 properties contradict the profile", () => {
    ...
    const fingerprint: DeviceFingerprint = {
      ...known,
      services: [{ ...known.services[0]!, characteristics: [{ ...known.services[0]!.characteristics[0]!, properties: { read: false, notify: false, indicate: false, write: true, writeWithoutResponse: false } }] }],
    };
    expect(coolLedUxDriver.resolveProfile(fingerprint)).toBeNull();
    expect(new DriverRegistry(builtInDrivers).match(fingerprint).selected).toBeNull();
  ...
  it("blocks equally plausible drivers as ambiguous", () => {
    const clone = { ...coolLedXDriver, id: "coolledx-clone", match: (fingerprint: DeviceFingerprint) => ({ ...coolLedXDriver.match(fingerprint), driverId: "coolledx-clone" }) };
    const selection = new DriverRegistry([coolLedXDriver, clone]).match(unknownSharedTransport());
    expect(selection.ambiguous).toBe(true);
    expect(selection.selected).toBeNull();
  });
});
...
describe("the active probe still resolves an unknown display", () => {
  it("resolves CoolLEDUX after verified probe evidence", () => {
    ...
    const selection = registry.resolve("coolledux", unknownSharedTransport(), "valid structured 0x1F response");
    expect(selection.selected?.id).toBe("coolledux");
    expect(selection.ambiguous).toBe(false);
