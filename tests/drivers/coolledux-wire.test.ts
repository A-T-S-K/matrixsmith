...
import { buildAnnouncePacket, buildDataChunkPackets, crc32Custom, lzssCompress, lzssCompressSafe, lzssDecompress, UX_PACKAGE_SIZE, xorAll } from "../../src/drivers/coolledux/wire";
...
describe("CoolLEDUX custom CRC32", () => {
  it.each(Object.entries(conformance.crc32))("matches the pinned reference for %s", (_name, vector) => {
  ...
  it("differs from standard CRC-32 (no final complement)", () => {
    ...
    expect(crc32Custom(new TextEncoder().encode("123456789"))).not.toBe(0xcbf43926);
  });
});

describe("CoolLEDUX LZSS", () => {
  it.each(Object.entries(conformance.lzss))("bit-matches the reference compressor for %s", (_name, vector) => {
    expect(toHex(lzssCompress(fromHex(vector.inputHex)))).toBe(vector.compressedHex);
  });

  it.each(Object.entries(conformance.lzss))("bit-matches the reference safe compressor for %s", (_name, vector) => {
    expect(toHex(lzssCompressSafe(fromHex(vector.inputHex)))).toBe(vector.safeHex);
  });

  it.each(Object.entries(conformance.lzss))("round-trips %s through the decoder", (_name, vector) => {
    const input = fromHex(vector.inputHex);
    expect(toHex(lzssDecompress(lzssCompress(input)))).toBe(vector.inputHex);
    expect(toHex(lzssDecompress(lzssCompressSafe(input)))).toBe(vector.inputHex);
  });

  it("decodes reference-produced streams identically", () => {
    for (const vector of Object.values(conformance.lzss)) {
      expect(toHex(lzssDecompress(fromHex(vector.compressedHex)))).toBe(vector.inputHex);
      expect(toHex(lzssDecompress(fromHex(vector.safeHex)))).toBe(vector.inputHex);
    }
  });
...
describe("program announce packet", () => {
  ...
  it("matches the reference announce bytes exactly", () => {
    expect(toHex(buildAnnouncePacket(fromHex(vector.programHex), vector.index, vector.count, vector.showCount))).toBe(vector.packetHex);
  });
  ...
  it("embeds CRC and uncompressed length big-endian after opcode 0x02", () => {
    ...
    const crc = ((payload[1]! << 24) | (payload[2]! << 16) | (payload[3]! << 8) | payload[4]!) >>> 0;
    expect(crc).toBe(vector.crc >>> 0);
    const length = ((payload[5]! << 24) | (payload[6]! << 16) | (payload[7]! << 8) | payload[8]!) >>> 0;
...
describe("program data chunks", () => {
  ...
  it("matches the reference chunk packets exactly", () => {
    const chunks = buildDataChunkPackets(fromHex(vector.compressedHex), vector.chunkSize);
    expect(chunks.map((chunk) => toHex(chunk.packet))).toEqual(vector.packetsHex);
  });
  ...
  it("encodes chunk metadata and XOR checksum per the layout", () => {
    ...
    for (const [index, chunk] of chunks.entries()) {
      ...
      const total = ((payload[2]! << 24) | (payload[3]! << 16) | (payload[4]! << 8) | payload[5]!) >>> 0;
...
describe("envelope escaping conformance", () => {
  it.each(Object.entries(conformance.envelope))("matches the reference envelope for %s", async (_name, vector) => {
    const { encodeEnvelope } = await import("../../src/drivers/coolled/common/envelope");
    expect(toHex(encodeEnvelope(fromHex(vector.payloadHex)))).toBe(vector.packetHex);
  });
});
