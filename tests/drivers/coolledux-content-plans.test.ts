...
describe("CoolLEDUX content plans", () => {
  it("compiles verified routine ShowFrame into an honest persistent multi-packet plan", () => {
    ...
    expect(plan.validation).toBe("verified");
    ...
    expect(plan.recoveryNotes.join(" ")).toContain(
      "replaces the stored display program",
    );
  });
  ...
  it("attaches 60 ms pacing metadata to every packet without sleeping in the codec", () => {
    ...
    expect(plan.packets.every((packet) => packet.delayAfterMs === 60)).toBe(
      true,
    );
  });
  ...
  it("derives dimensions from the profile and rejects mismatched content", () => {
    expect(() =>
      coolLedUxDriver.plan(
        { type: "ShowFrame", frame: new Framebuffer(64, 16) },
        context,
      ),
    ).toThrow(/DeviceProfile/);
    expect(() =>
      coolLedUxDriver.plan(
        { type: "ShowFrame", frame: new Framebuffer(32, 8) },
        context,
      ),
    ).toThrow(/DeviceProfile/);
  });
  ...
  it("requires the rendered framebuffer for ShowText", () => {
    expect(() =>
      coolLedUxDriver.plan({ type: "ShowText", text: "HI" }, context),
    ).toThrow(/rendered Framebuffer/);
  });

  it("compiles ShowAnimation with per-frame delays preserved in metadata", () => {
    const plan = coolLedUxDriver.plan(
      { type: "ShowAnimation", sequence: diagnosticAnimation(32, 16) },
      context,
    );
    expect(plan.metadata.contentType).toBe("animation");
  ...
  it("compiles ShowGif with raw bytes and warns via geometry checks", () => {
    ...
    const plan = coolLedUxDriver.plan(
      { type: "ShowGif", gifBytes: gif, width: 8, height: 16 },
      context,
    );
    expect(plan.metadata.contentType).toBe("gif");
    expect(() =>
      coolLedUxDriver.plan(
        { type: "ShowGif", gifBytes: gif, width: 64, height: 16 },
        context,
      ),
    ).toThrow(/exceeds/);
  });
  ...
  it("presents each capability at the confidence its evidence supports", () => {
    const byId = new Map(
      coolLedUxDriver
        .capabilities(iledHat31aeProfile)
