...
import {
  ...
  escapeCoolLedBytes,
  frameCoolLedPayload,
} from "../../src/drivers/coolledx/protocol";
...
describe("CoolLEDX framing", () => {
  it("escapes only 01, 02, and 03", () => {
    expect(hex(escapeCoolLedBytes(Uint8Array.of(0, 1, 2, 3, 4)))).toBe(
      "00 02 05 02 06 02 07 04",
    );
  });

  it("frames a generic payload with an escaped big-endian length", () => {
    expect(hex(frameCoolLedPayload(Uint8Array.of(0x08, 0x10)))).toBe(
      "01 00 02 06 08 10 03",
    );
  });
