...
describe("raster strategy routing", () => {
  it("routes ShowFrame through the profile's own preferred strategy", () => {
    ...
    const plan = coolLedUxDriver.plan(
      { type: "ShowFrame", frame },
      { profile: iledHat31aeProfile, fingerprint, source: "live" },
    );
  ...
  it("falls back to graffiti only for a profile with no resolved preference", () => {
    ...
    const plan = coolLedUxDriver.plan(
      { type: "ShowFrame", frame },
      { profile: unresolved, fingerprint, source: "live" },
    );
  ...
  it("routes ShowFrame through a validated animation-single-frame strategy", () => {
    const plan = coolLedUxDriver.plan(
      { type: "ShowFrame", frame },
      {
        ...
        fingerprint,
        source: "live",
        rasterStrategy: "animation-single-frame",
      },
    );
    expect(plan.metadata.rasterStrategy).toBe("animation-single-frame");
    expect(plan.metadata.contentType).toBe("animation");
  ...
  it("compiles distinct programs per strategy from the same logical frame", () => {
    ...
    expect(pair.programBytes.length).toBeGreaterThan(
      single.programBytes.length,
    );
...
describe("profile quirks", () => {
  it("ships the physically characterized iLedHat quirks", () => {
    ...
    expect(ILEDHAT_QUIRKS.graffitiBlack).toEqual({
      state: "true-black",
      basis: "observed",
    });
    expect(ILEDHAT_QUIRKS.animationBlack).toEqual({
      state: "true-black",
      basis: "observed",
    });
    // The high nibble was swept and drove nothing; there is no fourth channel
    // for a white emitter to live in.
    expect(ILEDHAT_QUIRKS.whiteChannel).toEqual({
      state: "absent",
      basis: "observed",
    });
    expect(ILEDHAT_QUIRKS.preferredRasterStrategy).toBe(
  ...
  it("drops Graffiti as a normal static candidate after physically rejecting it", () => {
    expect(ILEDHAT_QUIRKS.rasterStrategyCandidates).toEqual([
      "animation-single-frame",
    ...
    expect(COOLLEDUX_DEFAULT_QUIRKS.rasterStrategyCandidates).toContain(
      "graffiti",
    );
  ...
  it("declares every strategy candidate as a real strategy", () => {
    for (const candidate of ILEDHAT_QUIRKS.rasterStrategyCandidates)
      expect(isRasterStrategy(candidate)).toBe(true);
  });
});

describe("encodeRawWordRegion", () => {
  it("emits column-major big-endian words byte-for-byte", () => {
    const words = Uint16Array.of(0x1234, 0xabcd, 0x0004, 0xf000); // 2×2 grid, row-major
    const encoded = encodeRawWordRegion(words, 2, 2, 0, 2);
    expect([...encoded]).toEqual([
      0x12, 0x34, 0x00, 0x04, 0xab, 0xcd, 0xf0, 0x00,
    ]);
  });

  it("rejects out-of-range regions and mismatched grids", () => {
    expect(() => encodeRawWordRegion(Uint16Array.of(0), 2, 2, 0, 2)).toThrow(
      RangeError,
    );
    expect(() => encodeRawWordRegion(new Uint16Array(4), 2, 2, 1, 2)).toThrow(
      RangeError,
    );
  });
});
