...
import {
  coolLedUxGuidedTests,
  ILEDHAT_GUIDED_TESTS,
} from "../../src/drivers/coolledux/guided-tests";
...
describe("guided test applicability", () => {
  it("exposes the iLedHat suite to the iLedHat profile", () => {
    ...
    expect(tests.map((test) => test.id)).toEqual(
      ILEDHAT_GUIDED_TESTS.map((test) => test.id),
    );
  });
  ...
  it("does not expose iLedHat-specific tests to another CoolLEDUX profile", () => {
    ...
    for (const test of tests)
      expect(ILEDHAT_GUIDED_TESTS.map((entry) => entry.id)).not.toContain(
        test.id,
      );
  ...
  it("exposes nothing to a non-CoolLEDUX profile", () => {
    expect(
      coolLedUxGuidedTests({ ...otherCoolLedUxProfile, driverId: "coolledx" }),
    ).toHaveLength(0);
  });
});
