...
async function connected(): Promise<MatrixController> {
...
describe("the normal-image smoke test uses the production path", () => {
  it("plans a plain ShowFrame through the profile's own strategy", async () => {
    ...
    const expected = compileAnimationStaticFrame(orientationPattern(32, 16), "single");
    expect(plan.metadata.crc32).toBe(`0x${expected.crc32.toString(16).padStart(8, "0").toUpperCase()}`);
    const createPlan = controller.plan({ type: "ShowFrame", frame: orientationPattern(32, 16) });
    expect(plan.metadata.crc32).toBe(createPlan.metadata.crc32);
  }, 30000);

  it("is optional and never the automatic next step", async () => {
    const controller = await connected();
    const availability = controller.guidedTests().find((entry) => entry.test.id === "coolledux-normal-image-smoke")!;
  ...
  it("promotes image.rendering only on a clean, sufficiently long observation", async () => {
    const controller = await connected();
    expect(controller.claims().find((claim) => claim.id === "image.rendering")?.status).toBe("unknown");
    const completed = controller.recordGuidedTestObservations("coolledux-normal-image-smoke", clean(), []);
    expect(completed.status).toBe("passed");
    const state = controller.claims().find((claim) => claim.id === "image.rendering");
  ...
  it("stays retryable when the observation was too short to judge", async () => {
    const controller = await connected();
    const completed = controller.recordGuidedTestObservations("coolledux-normal-image-smoke", clean(7000), []);
  ...
  it("rejects the image path when a normal image does not hold still", async () => {
    const controller = await connected();
    const completed = controller.recordGuidedTestObservations("coolledux-normal-image-smoke", [
      { kind: "boolean", fieldId: "initial-correct", value: "yes" },
      timer("image-visible", 900),
    ...
    expect(controller.claims().find((claim) => claim.id === "image.rendering")?.status).toBe("rejected");
  }, 30000);

  it("does not judge colour quality", async () => {
    const controller = await connected();
    controller.recordGuidedTestObservations("coolledux-normal-image-smoke", clean(), []);
    ...
    expect(controller.claims().find((claim) => claim.id === "pixel.color-calibration")?.status).toBe("unresolved");
  }, 30000);
});
