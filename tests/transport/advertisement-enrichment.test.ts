...
const HINTS = { filters: [{ services: ["0000fff0-0000-1000-8000-00805f9b34fb"] }], optionalServices: ["0000fff0-0000-1000-8000-00805f9b34fb"], optionalManufacturerData: [0x31ae] };
...
function makeDevice(extra: Record<string, unknown> = {}): BluetoothDevice {
  ...
  const device = Object.assign(new EventTarget(), { id: "browser-id", name: "iLedHat" }, extra) as unknown as BluetoothDevice;
  const service = { uuid: "0000fff0-0000-1000-8000-00805f9b34fb", isPrimary: true, device, getCharacteristics: async () => [characteristic] } as unknown as BluetoothRemoteGATTService;
  const server = { connected: true, getPrimaryServices: async () => [service] } as unknown as BluetoothRemoteGATTServer;
  Object.defineProperty(device, "gatt", { value: { connect: async () => server, disconnect: () => undefined, connected: true } });
  return device;
}
...
describe("advertisement enrichment", () => {
  it("captures a structured observation without fabricating raw bytes", async () => {
      ...
      watchAdvertisements: async () => {
        queueMicrotask(() => {
          const event = Object.assign(new Event("advertisementreceived"), {
            uuids: ["0000fff0-0000-1000-8000-00805f9b34fb"], rssi: -52, txPower: 4, name: "iLedHat",
            manufacturerData: new Map([[0x31ae, new DataView(Uint8Array.of(0x5e, 0xea, 0x07).buffer)]]),
            serviceData: new Map(),
          });
    ...
    expect(observation!.manufacturerData).toEqual([{ companyId: 0x31ae, dataHex: "5E EA 07" }]);
  ...
  it("returns null when watchAdvertisements is unsupported and never blocks the connection", async () => {
    ...
    Object.defineProperty(globalThis, "navigator", { configurable: true, value: { bluetooth: { requestDevice: async () => device } } });
    const transport = new WebBluetoothTransport(new TraceRecorder());
    const fingerprint = await transport.selectAndConnect({ mode: "registered", hints: HINTS });
...
describe("previously authorized reconnect", () => {
  it("reconnects by device id without opening a chooser", async () => {
    ...
    Object.defineProperty(globalThis, "navigator", {
      configurable: true,
      value: { bluetooth: { requestDevice: async () => { chooserOpened = true; return device; }, getDevices: async () => [device] } },
    });
    const transport = new WebBluetoothTransport(new TraceRecorder());
    const fingerprint = await transport.reconnectAuthorized("browser-id", { mode: "registered", hints: HINTS });
  ...
  it("throws a clear error when the device is no longer authorized (caller falls back to the chooser)", async () => {
    Object.defineProperty(globalThis, "navigator", { configurable: true, value: { bluetooth: { requestDevice: async () => makeDevice(), getDevices: async () => [] } } });
    const transport = new WebBluetoothTransport(new TraceRecorder());
    await expect(transport.reconnectAuthorized("browser-id", { mode: "registered", hints: HINTS })).rejects.toThrow(/chooser/i);
  });

  it("throws when getDevices is unsupported", async () => {
    Object.defineProperty(globalThis, "navigator", { configurable: true, value: { bluetooth: { requestDevice: async () => makeDevice() } } });
    const transport = new WebBluetoothTransport(new TraceRecorder());
    await expect(transport.reconnectAuthorized("browser-id", { mode: "registered", hints: HINTS })).rejects.toThrow(/does not support/i);
  });
});
