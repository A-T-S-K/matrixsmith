...
function ContentGate({ snapshot, store }: { snapshot: AppSnapshot; store: MatrixStore }): JSX.Element {
  return <article class="panel content-gate">
    <div class="panel-title"><div><span class="panel-kicker">EXPERIMENTAL · NOT YET VALIDATED HERE</span><h2>Content is preview-only</h2></div><StatusBadge tone="warn">Validation required</StatusBadge></div>
    <p>{snapshot.content.allowedReason}</p>
    <p class="fineprint">Sending content replaces the display's stored program. MatrixSmith validates the static framebuffer with a guided diagnostic before offering normal content controls.</p>
    <button class="primary" disabled={snapshot.busy !== null || !snapshot.liveConnected} onClick={() => { store.setView("diagnose"); store.startValidation("coolledux-validate-static-frame"); }}>Validate static framebuffer</button>
  </article>;
}
...
function ContentSection({ snapshot, store }: { snapshot: AppSnapshot; store: MatrixStore }): JSX.Element {
  ...
  const canSend = content.allowed && snapshot.busy === null;
  const sendTitle = content.allowed ? "" : content.allowedReason;
  ...
  return <section>
    ...
    {!content.allowed && <ContentGate snapshot={snapshot} store={store}/>}
    <div class="content-grid">
      <article class="panel">
        <div class="panel-title"><div><span class="panel-kicker">TEXT</span><h2>Rendered text</h2></div><StatusBadge tone={content.allowed ? "good" : "warn"}>{content.allowed ? "Validated pipeline" : "Experimental"}</StatusBadge></div>
        ...
        <button class="primary" disabled={!canSend || !settings.text.trim()} title={sendTitle} onClick={() => store.requestSendText()}>Send text…</button>
      </article>
      <article class="panel">
        <div class="panel-title"><div><span class="panel-kicker">IMAGE</span><h2>Image</h2></div><StatusBadge tone={content.allowed ? "good" : "warn"}>{content.allowed ? "Validated pipeline" : "Experimental"}</StatusBadge></div>
        ...
        <button class="primary" disabled={!canSend || !content.image} title={sendTitle} onClick={() => store.requestSendImage()}>Send image…</button>
      </article>
      <article class="panel">
        <div class="panel-title"><div><span class="panel-kicker">ANIMATION</span><h2>Animation</h2></div><StatusBadge tone="warn">Experimental</StatusBadge></div>
        ...
        <button class="primary" disabled={!canSend || (content.animationChoice === "scroll-text" && !settings.text.trim())} title={sendTitle} onClick={() => store.requestSendAnimation()}>Send animation…</button>
      </article>
      <article class="panel">
        ...
        <button class="primary" disabled={!canSend || !content.gif} title={sendTitle} onClick={() => store.requestSendGif()}>Send GIF…</button>
      </article>
    </div>
  </section>;
}
