 ...
 */
export function AnswerControls({ spec, value, store, autoAdvance }: {
  ...
  if (spec.kind === "boolean") {
    ...
    return <div class="answer-buttons">{(["yes", "no", "unsure"] as const).map((option) => <button
      type="button"
      class={current === option ? `active ${option}` : ""}
      aria-pressed={current === option}
      onClick={() => { store.setGuidedObservation({ kind: "boolean", fieldId: spec.id, value: option }); autoAdvance?.(); }}
    >{option === "yes" ? "Yes" : option === "no" ? "No" : "Unsure"}</button>)}</div>;
  }
  if (spec.kind === "choice") {
    ...
    return <>
      <div class="answer-buttons choice-buttons">
        {spec.options.map((option) => <button
          type="button"
          class={current === option.id ? "active" : ""}
          aria-pressed={current === option.id}
          onClick={() => { store.setGuidedObservation({ kind: "choice", fieldId: spec.id, optionId: option.id }); autoAdvance?.(); }}
        >{option.label}</button>)}
        {spec.allowOther && <button
          type="button"
          class={current === "other" ? "active" : ""}
          aria-pressed={current === "other"}
          onClick={() => store.setGuidedObservation({ kind: "choice", fieldId: spec.id, optionId: "other" })}
      ...
      {current === "other" && <input
        ...
        value={otherText}
        onInput={(event) => store.setGuidedObservation({ kind: "choice", fieldId: spec.id, optionId: "other", otherText: (event.currentTarget as HTMLInputElement).value })}
      />}
    </>;
  }
  if (spec.kind === "duration") {
    return <p class="fineprint">{value?.kind === "duration"
      ? `${formatDuration(value.milliseconds)} (${value.measuredBy === "matrixsmith-timer" ? "from your mark, timed by MatrixSmith" : "estimate"})`
      : "Recorded from the timed observation — nothing to enter here."}</p>;
  }
  if (spec.kind === "number") {
    const current = value?.kind === "number" ? String(value.value) : "";
    return <p><input
      type="number" value={current} aria-label={spec.prompt}
      onInput={(event) => { const parsed = Number((event.currentTarget as HTMLInputElement).value); if (Number.isFinite(parsed)) store.setGuidedObservation({ kind: "number", fieldId: spec.id, value: parsed }); }}
    />{spec.unit && <small> {spec.unit}</small>}</p>;
  }
  const text = value?.kind === "note" ? value.text : "";
  return <textarea
    rows={2} value={text} aria-label={spec.prompt} placeholder="Optional"
    onInput={(event) => store.setGuidedObservation({ kind: "note", fieldId: spec.id, text: (event.currentTarget as HTMLTextAreaElement).value })}
  />;
}
