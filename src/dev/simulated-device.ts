...
const VALUE_INDEX = 5;
const DEVICE_INFO_CC =
  "0100301F0205CC00000000020509020504020700000500001E001000AE310020001007694C65644861740000000000000000000000000003";
const DEVICE_INFO_40 =
  "0100301F02054000000000020509020504020700000500001E001000AE310020001007694C65644861740000000000000000000000000003";
...
export class SimulatedIledHatTransport implements MatrixTransport {
  ...
  constructor(unit = "simulated-iledhat", unknown = false) {
    this.unit = unit;
  ...
  async selectAndConnect(
    ...
    this.connectionId = `simulated:${globalThis.crypto?.randomUUID?.() ?? Date.now()}`;
  ...
  async listAuthorizedDevices(): Promise<
  ...
  #respond(bytes: Uint8Array): Uint8Array | null {
    ...
    if (opcode === 0x1f)
      return parseHexBytes(
        this.brightness === 0xcc ? DEVICE_INFO_CC : DEVICE_INFO_40,
      );
    if (opcode === 0x04) {
      this.brightness = bytes[VALUE_INDEX] ?? this.brightness;
      return bytes.slice();
    }
    return null;
  }
