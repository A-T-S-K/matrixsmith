...
export function decodeCoolLedNotification(
  packet: Uint8Array,
  ...
  if (opcode === 0x08 && code === 0xfe)
    return {
      ...
      success: false,
      summary:
        "Classic brightness response value 0xFE; rejection/error-like, exact semantics unmapped.",
      payloadHex: packetHex(payload),
      fields: { value: code, semanticsMapped: false },
    };
  if (code === undefined || !(code in ERROR_NAMES))
    return {
      family: "CoolLEDX",
      kind: "command-response",
      opcode,
      summary: `Unmapped response for opcode 0x${opcode.toString(16).padStart(2, "0")}.`,
      payloadHex: packetHex(payload),
      fields: {},
      ...(payload.length > 1
