 ...
 */
const ILEDHAT_PROFILE_EVIDENCE: readonly ClaimEvidence[] = Object.freeze<ClaimEvidence[]>([
  ...
  { claimId: "raster.tiling", status: "verified", scope: "built-in-profile", provenance: "observed", summary: "Four 8-column tiles reconstructed a recognizable full 32×16 diagnostic raster with no seams in the 2026-09-01 run." },
  { claimId: "raster.orientation", status: "verified", scope: "built-in-profile", provenance: "observed", summary: "Corner positions and placement were correct — not rotated or mirrored — in the 2026-09-01 run." },
  ...
  {
    claimId: "graffiti.playback-stability", status: "rejected", scope: "built-in-profile", provenance: "observed",
    summary: "Both justified Graffiti playback configurations move on this panel. stayTime=3 held visibly still for approximately 3.6 s and stayTime=0 for approximately 1.0 s, each well below the 15 s required to call a raster stable, and each then began moving. Graffiti is not a usable static-image route here. Only these two configurations were tested; the stayTime byte's own semantics remain unknown.",
    metrics: { stayTime3HoldMs: 3600, stayTime0HoldMs: 1000 },
    details: { stayTime3: "moves", stayTime0: "moves" },
  },
  {
    claimId: "graffiti.black-semantics", status: "verified", scope: "built-in-profile", provenance: "observed",
    summary: "Literal raw 0x0000 is genuinely off/black on the Graffiti path of this exact iLedHat; raw 0x0004 renders as a visibly dim blue. The upstream 0x0000-is-white sentinel does NOT apply to this panel, so its 0x0004 workaround must not be substituted here.",
    details: { zeroBehavior: "true-black", workaroundAppearance: "dim-blue" },
  },
  { claimId: "graffiti.color-mapping", status: "verified", scope: "built-in-profile", provenance: "observed", summary: "Logical red, green and blue reach the intended physical colors on the Graffiti path; the same encoder feeds both content paths. Perceived white neutrality is tracked separately as color calibration." },
  ...
  { claimId: "animation.black-semantics", status: "verified", scope: "built-in-profile", provenance: "observed", summary: "Animation-path literal 0x0000 background is genuinely off/black on this iLedHat.", details: { zeroBehavior: "true-black" } },
  ...
  {
    claimId: "animation.static-single-frame", status: "verified", scope: "built-in-profile", provenance: "observed",
    summary: "A one-frame Animation program rendered the full tiled raster and held completely still for approximately 16.8 s measured from full-raster-visible, with the background genuinely off, tiles aligned, and no flicker or reset.",
    metrics: { visibleStaticHoldMs: 16800 },
  },
  {
    claimId: "animation.static-identical-pair", status: "verified", scope: "built-in-profile", provenance: "observed",
    summary: "A two-identical-frame Animation program held completely still for approximately 16.2 s from full-raster-visible, with the background genuinely off, tiles aligned, and no flicker or reset. Verified as a fallback; the one-frame variant is the preferred route.",
    metrics: { visibleStaticHoldMs: 16200 },
  },
  {
    claimId: "pixel.channel-map", status: "verified", scope: "built-in-profile", provenance: "observed",
    summary: "Physically confirmed by single-channel patch observation: byte0 low nibble drives red, byte1 high nibble green, byte1 low nibble blue.",
    details: { observedMap: "0x0F00→red, 0x00F0→green, 0x000F→blue", matchesRgb444: true },
  },
  { claimId: "pixel.encoder-correctness", status: "verified", scope: "built-in-profile", provenance: "observed", summary: "The observed raw mapping matches the RGB444 ordering MatrixSmith's encoder emits, so logical colors reach the intended physical channels without correction." },
  { claimId: "pixel.fourth-channel", status: "rejected", scope: "built-in-profile", provenance: "observed", summary: "High-nibble-only probes 0x1000 through 0xF000 were all observed off. The byte0 high nibble drives no fourth physical emitter on this panel." },
  { claimId: "pixel.white-channel", status: "rejected", scope: "built-in-profile", provenance: "observed", summary: "No fourth channel exists on this panel, so there is no dedicated white emitter. Content must never assume an RGBW encoding here." },
  ...
  { claimId: "pixel.color-calibration", status: "unresolved", scope: "built-in-profile", provenance: "observed", summary: "Logical R/G/B reach the correct physical colors and no single channel was judged obviously brighter or dimmer, but RGB-max white appears tinted rather than neutral. Calibration is optional and does not block image or text use." },
...
const SOURCE_REFERENCE_EVIDENCE: readonly ClaimEvidence[] = Object.freeze([
  ...
  { claimId: "graffiti.black-semantics", status: "source-supported", scope: "source-reference", provenance: "source-derived", summary: "Upstream reference hardware renders a literal Graffiti 0x0000 pixel as bright white, motivating the 0x0004 off workaround (coolledux-ble@4f5656d). Physically contradicted on the iLedHat, where 0x0000 is genuinely black." },
