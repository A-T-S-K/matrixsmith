  ...
  steps: [
    {
      ...
      purpose:
        "Does a still image appear on the display at all, and correctly?",
    ...
    },
    {
      ...
      purpose:
        "Does the image hold steady, and if not, does the one justified alternative setting fix it?",
    ...
    },
    {
      ...
      purpose:
        "Are dark pixels genuinely off, or does this panel need the inherited workaround?",
      ...
      skipWhen(evidence) {
        ...
        const graffiti = assessment.strategies.find(
          (entry) => entry.strategy === "graffiti",
        );
        if (
          graffiti?.verdict === "not-viable" &&
          !trustedOrRejected("graffiti.black-semantics", evidence)
        ) {
    ...
    },
    {
      ...
      purpose:
        "If the native path cannot hold a still image, can the animation path?",
      ...
      skipWhen(evidence) {
        ...
        if (graffiti?.verdict === "viable")
          return "The native still-image path works, so no fallback was needed.";
    ...
    },
    {
      ...
      purpose:
        "Which parts of a pixel drive which physical colors — required before any image or text is trustworthy.",
