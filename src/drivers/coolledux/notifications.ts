...
export function decodeCoolLedUxNotification(
  packet: Uint8Array,
): DecodedNotification | null {
  let payload: Uint8Array;
  try {
    payload = decodeEnvelope(packet).payload;
  } catch (error) {
    return {
      family: "CoolLEDUX",
      kind: "malformed-envelope",
      summary: "Malformed CoolLED envelope",
      payloadHex: "",
      fields: {},
  ...
  if (opcode === COOLLEDUX_OPCODES.brightness && payload.length >= 2) {
    ...
    return {
      ...
      kind: "command-echo",
      opcode,
      summary: `Brightness command echoed at raw level ${level}.`,
      payloadHex,
      success: true,
      fields: { brightnessRaw: level },
      ...(payload.length > 2
        ? { unknownTailHex: packetHex(payload.slice(2)) }
        : {}),
    };
  }
  if (opcode === COOLLEDUX_OPCODES.deviceInfo) {
    if (payload.length < 3)
      return {
        family: "CoolLEDUX",
        kind: "malformed-device-info",
        opcode,
        summary: "Device-info response lacks the confirmed three-byte prefix.",
        payloadHex,
        fields: {},
        unknownTailHex: packetHex(payload.slice(1)),
      };
    ...
    return {
      ...
      kind: "device-info",
      opcode,
      summary: `Device info: power raw ${powerRaw}, brightness raw ${brightnessRaw}.`,
      payloadHex,
      success: true,
      fields: {
        payloadLength: payload.length,
        powerRaw,
        powerOn: powerRaw === 1 ? true : null,
        brightnessRaw,
      },
  ...
  if (opcode === 0x03 && payload.length === 5) {
    ...
    return {
      ...
      payloadHex,
      status: statusRaw,
      fields: { reservedRaw, chunkIndex, statusRaw },
    };
  }
  ...
  return {
    ...
    summary:
      value === undefined
        ? `Response for opcode 0x${opcode.toString(16).padStart(2, "0")}.`
        : `Response for opcode 0x${opcode.toString(16).padStart(2, "0")} with value 0x${value.toString(16).padStart(2, "0")}.`,
    payloadHex,
