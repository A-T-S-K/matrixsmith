...
import {
  ...
  choiceAnswer,
  durationAnswer,
  measuredDurationAnswer,
  type ClaimUpdate,
  type GuidedTestDefinition,
  type GuidedTestInterpretation,
  type GuidedTestTimer,
...
const graffitiBlackTest: GuidedTestDefinition = {
  ...
  consequence: PERSISTENT_CONSEQUENCE,
  about: {
    question:
      "Does this iLedHat actually require the blue-ish nonzero workaround for black static-image pixels?",
    whyRelevant:
      "MatrixSmith currently inherits a blue-ish black workaround from another CoolLEDUX device; this exact iLedHat has not been tested for that behavior.",
    whatMatrixSmithDoes:
      "Uploads a fixed pattern with alternating 8-column regions: literal raw 0x0000 (columns 0–7 and 16–23) and the raw 0x0004 workaround (columns 8–15 and 24–31), bypassing the normal off-color substitution. One bright marker pixel tops each tile.",
    whatChangesOnDevice:
      "The stored display program is replaced with this diagnostic pattern.",
    estimatedObservationTime: "About 1 minute.",
    possibleOutcomes: [
      {
        outcome: "The 0x0000 regions are off/black",
        learns:
          "The upstream Graffiti white-sentinel quirk does not apply here; true black is available on the static-image path.",
      },
      {
        outcome: "The 0x0000 regions are bright white",
        learns:
          "The white-sentinel quirk is directly corroborated on this profile; the workaround stays necessary.",
      },
      {
        outcome: "Something else / unsure",
        learns:
          "Black semantics stay uncharacterized; the observation is still recorded as evidence.",
      },
    ],
    preTransferNote:
      "Judge the initial image immediately. This test isolates black/off pixel encoding. Any later movement is a separate playback question.",
    observeInstructions:
      "Look at the two kinds of vertical regions right after the upload. Ignore the single bright marker pixel at the top of each 8-column tile.",
    technicalDetails: [
  ...
  observation: [
    {
      kind: "choice",
      id: "zero-appearance",
      regionId: "black-candidate-0",
      prompt: "What color are the highlighted zones on the display?",
      options: [
        { id: "off-black", label: "Off / black" },
        { id: "bright-white", label: "Bright white" },
        { id: "dim-blue", label: "Dim blue" },
        { id: "another-color", label: "Another color" },
      ],
      allowOther: true,
    },
    {
      kind: "choice",
      id: "workaround-appearance",
      regionId: "workaround-1",
      prompt: "What color are the highlighted zones on the display?",
      options: [
        { id: "off-black", label: "Off / black" },
        { id: "dim-blue", label: "Dim blue" },
  ...
  interpret(values) {
    ...
    if (zero === "off-black") {
      ...
      established.push(
        "Graffiti-path literal 0x0000 is genuinely off/black on this exact device.",
      );
      rejected.push(
        "The upstream white-sentinel quirk (0x0000 → bright white) does not reproduce on this profile.",
      );
      updates.push({
        claimId: "graffiti.black-semantics",
        status: "verified",
        summary:
          "Literal 0x0000 observed off/black on the Graffiti path; the upstream white-sentinel quirk does not apply to this profile. Built-in profile behavior is not changed automatically.",
        details: {
          zeroBehavior: "true-black",
      ...
      status = "passed";
      summary =
        "Literal Graffiti 0x0000 renders bright white: the upstream quirk is directly corroborated on this iLedHat.";
      established.push(
        "The Graffiti white-sentinel quirk is directly observed on this profile; the 0x0004 workaround remains necessary.",
      );
      updates.push({
        claimId: "graffiti.black-semantics",
        status: "verified",
        summary:
          "Literal 0x0000 observed bright white on the Graffiti path; the white-sentinel workaround is required on this profile.",
        details: {
          zeroBehavior: "white-sentinel",
          ...(workaround ? { workaroundAppearance: workaround } : {}),
        },
      });
    } else if (zero !== null) {
      ...
      unknowns.push(
        "Graffiti 0x0000 semantics remain uncharacterized: the observed appearance matches neither hypothesis.",
      );
      updates.push({
        claimId: "graffiti.black-semantics",
        status: "unresolved",
        summary: `Raw 0x0000 regions observed as "${zero}" — neither off/black nor bright white.`,
        details: {
          zeroBehavior: "other",
          observedAppearance: zero,
          ...(workaround ? { workaroundAppearance: workaround } : {}),
        },
      });
    } else {
    ...
    if (workaround && workaround !== "off-black")
      established.push(
        `The raw 0x0004 workaround regions appeared "${workaround}" — visible rather than fully off.`,
      );
    else if (workaround === "off-black")
      established.push("The raw 0x0004 workaround regions appeared off/black.");
    return {
      status,
      established,
      rejected,
      ...
      nextHint:
        "Black behavior is one prerequisite of static viability — it does not by itself make the Graffiti strategy usable. Continue with pixel/channel characterization.",
    };
  },
};
  ...
  return [
    {
      kind: "boolean",
      id: "initial-correct",
      prompt: "Did the full test image appear correctly at first?",
    },
    {
      kind: "duration",
      id: "image-visible",
      prompt: "When was the complete image visible? (measured by the timer)",
      required: false,
    },
    {
      kind: "boolean",
      id: "moved",
      prompt: "Did the image start moving at any point?",
      required: false,
    },
    {
      kind: "duration",
      id: "movement-start",
      prompt: "When did movement begin? (measured by the timer)",
      required: false,
    },
    {
      kind: "duration",
      id: "observation-end",
      prompt:
        "When did you stop watching the still image? (measured by the timer)",
      required: false,
    },
    {
      kind: "choice",
      id: "motion-description",
      prompt: "If it moved, what did the motion look like?",
      required: false,
      options: [
    ...
    },
    {
      ...
      id: "tiles-present",
      regionId: "tile-1",
      prompt: "Were all four highlighted sections present on the display?",
      required: false,
    },
    {
      kind: "boolean",
      id: "orientation-correct",
      regionId: "corner-top-left",
      prompt:
        "Was the red corner in the highlighted position — image the right way up and not mirrored?",
      required: false,
    },
    {
      kind: "boolean",
      id: "seams",
      regionId: "seam-1",
      prompt: "Did you notice a step or gap at the highlighted joins?",
      required: false,
    },
    { kind: "note", id: "note", prompt: "Anything else worth recording?" },
  ];
...
function validateTimeline(
  ...
  if (t1 !== null && t2 !== null && t2 < t1)
    errors.push("Movement cannot begin before the full image was visible.");
  if (t1 !== null && end !== null && end < t1)
    errors.push(
      "The observation cannot end before the full image was visible.",
    );
  return errors;
}
 ...
 * yes/no answer.
 */
function interpretTiming(
  values: readonly ObservationValue[],
  stayTime: number,
): GuidedTestInterpretation {
  ...
  const renderMetrics: Record<string, number> =
    t1 !== null ? { renderLatencyMs: t1 } : {};
  if (initial === "yes") {
    established.push(
      t1 !== null
    ...
    updates.push({
      ...
      status: "verified",
      summary: `Full tiled raster rendered correctly after upload ${parameterNote}${t1 !== null ? `; render latency ${formatDuration(t1)} (measured)` : ""}.`,
      ...(t1 !== null ? { metrics: { renderLatencyMs: t1 } } : {}),
    });
  } else if (initial === "no") {
    ...
    updates.push({
      claimId: "graffiti.initial-render",
      status: "rejected",
      summary: `The tiled raster failed to render correctly ${parameterNote}.`,
    });
  }
  ...
  if (tilesIssue) {
    rejected.push(
      "Tile sections were missing or misaligned during this observation.",
    );
    updates.push({
      claimId: "raster.tiling",
      status: "unresolved",
      summary: `Tile sections were reported missing or misaligned ${parameterNote}; tiling needs recharacterization.`,
    });
    ...
    updates.push({
      claimId: "raster.tiling",
      status: "verified",
      summary: `All tile sections observed present and aligned with no seams ${parameterNote}.`,
    });
  }
  ...
  if (orientation === "no") {
    ...
    updates.push({
      ...
      status: "unresolved",
      summary: `Orientation was reported wrong ${parameterNote}; orientation needs recharacterization.`,
    });
    ...
    updates.push({
      claimId: "raster.orientation",
      status: "verified",
      summary: `Orientation observed correct — not rotated or mirrored ${parameterNote}.`,
    });
  }
  ...
  if (moved === "no") {
    ...
    if (heldMs !== null && heldMs >= MINIMUM_STATIC_HOLD_MS) {
      ...
      established.push(
        `No movement within ${approximateSeconds(heldMs)} of observation — past the ${MINIMUM_STATIC_HOLD_MS / 1000}s stability threshold.`,
      );
      updates.push({
    ...
    const onset =
      heldMs !== null
        ...
        : t2 !== null
          ? `about ${approximateSeconds(t2).replace("~", "")} after the upload finished`
          : null;
    summary = onset
      ? `The image appeared correctly, then started moving ${onset}.`
      : "The image appeared correctly and then started moving.";
    rejected.push(
      onset
    ...
    updates.push({
      claimId: "graffiti.playback-stability",
      status: "unknown",
      summary: `Observation inconclusive ${parameterNote}.`,
    });
  }
  return {
    status,
...
const graffitiTimingTest: GuidedTestDefinition = {
  ...
  about: {
    question:
      "Does a static image stay still on this display, and if not, exactly when does it start moving?",
    whyRelevant:
      "The static-image raster previously rendered correctly and then began moving in a cycle. Measuring the onset precisely is the best evidence for what triggers it.",
    whatMatrixSmithDoes:
      "Uploads a deterministic high-contrast raster with the baseline playback bytes, then starts timing the moment the final program packet is accepted. Your taps are recorded as human observations, so the durations they produce are approximate.",
    whatChangesOnDevice:
      "The stored display program is replaced with the diagnostic raster.",
    estimatedObservationTime: "Watch for up to 15–20 seconds.",
    possibleOutcomes: [
      {
        outcome: "The image never moves",
        learns: "Graffiti is a viable static-image strategy at these settings.",
      },
      {
        outcome: "Movement starts after a measurable delay",
        learns:
          "The measured onset points at a playback parameter (like stayTime) as the trigger.",
      },
      {
        outcome: "The image is wrong from the start",
        learns: "The problem is rendering, not playback timing.",
      },
    ],
    observeInstructions:
      "Tap when the full image is visible, then tap the instant anything moves — or stop once it has stayed completely still for 15 seconds.",
    technicalDetails: [
  ...
  operation: {
    type: "ShowDiagnostic",
    diagnosticId: "graffiti-timing-probe",
...
const graffitiStayTimeTest: GuidedTestDefinition = {
  ...
  prerequisites: [{ claimId: "stored-program.upload", anyOf: ["verified"] }],
  ...
  about: {
    question:
      "Does the undocumented stayTime byte control when the static image starts moving?",
    whyRelevant:
      "The baseline measurement used stayTime=3 (the only value the upstream source ever uses, with unknown semantics). Changing exactly this one byte to 0 isolates its effect.",
    whatMatrixSmithDoes:
      "Uploads the identical raster and playback bytes as the baseline measurement, changing only stayTime from 3 to 0, then runs the same stopwatch observation.",
    whatChangesOnDevice:
      "The stored display program is replaced with the diagnostic raster.",
    estimatedObservationTime: "Watch for up to 15–20 seconds.",
    possibleOutcomes: [
      {
        outcome: "Behavior matches the baseline",
        learns:
          "stayTime is probably not the movement trigger at these values.",
      },
      {
        outcome: "Movement timing changes",
        learns:
          "stayTime directly influences playback; its scale can be characterized next.",
      },
      {
        outcome: "The image stays still",
        learns: "stayTime=0 is a static configuration for this panel.",
      },
    ],
    observeInstructions:
      "Watch the panel exactly as before and follow the same timer prompts.",
    technicalDetails: [
  ...
  return [
    {
      kind: "boolean",
      id: "initial-correct",
      prompt: "Did the image appear correctly at first?",
    },
    {
      kind: "duration",
      id: "image-visible",
    ...
    },
    {
      kind: "boolean",
      id: "moved",
      prompt: "Did the image move, flicker, or reset?",
      required: false,
    },
    {
      kind: "duration",
      id: "movement-start",
      prompt: "When did it move or reset? (measured by the timer)",
      required: false,
    },
    {
      kind: "duration",
    ...
    },
    {
      kind: "boolean",
      id: "background-off",
      prompt: "Is the background genuinely off/black?",
    },
    {
      kind: "boolean",
      id: "tiles-aligned",
      regionId: "seam-1",
      prompt:
        "Are the sections aligned, with no step or gap at the highlighted joins?",
    },
    {
      kind: "boolean",
      id: "flicker",
      prompt: "Did you notice any flicker or periodic reset?",
      required: false,
    },
    { kind: "note", id: "note", prompt: "Anything else worth recording?" },
  ];
...
function interpretAnimationStatic(
  values: readonly ObservationValue[],
    ...
    t1 !== null
      ? moved === "no" && end !== null
        ...
        : moved === "yes" && t2 !== null
          ...
          : null
      : null;
  const still: "yes" | "no" | null =
    moved === "yes"
      ? "no"
      : moved === "no" && heldMs !== null && heldMs >= MINIMUM_STATIC_HOLD_MS
        ? "yes"
        : null;
  ...
  const claimId =
    variant === "single"
      ? ("animation.static-single-frame" as const)
      : ("animation.static-identical-pair" as const);
  const updates: ClaimUpdate[] = [];
  const established: string[] = [];
  const rejected: string[] = [];
  const unknowns: string[] = [];
  if (background === "yes")
    established.push(
      "The literal 0x0000 background was off, re-confirming Animation black.",
    );
    ...
    rejected.push(
      "The literal 0x0000 background was NOT off — contradicting the earlier Animation true-black characterization.",
    );
    updates.push({
      claimId: "animation.black-semantics",
      status: "unresolved",
      summary: `The 0x0000 background was not off during the ${label} test, contradicting the earlier true-black observation; Animation black needs recharacterization.`,
    });
  }
  if (tiles === "yes") established.push("All four tiles were aligned.");
  if (flicker === "yes")
    established.push("Flicker or a periodic reset was observed and recorded.");
  const passed =
    initial === "yes" && still === "yes" && tiles !== "no" && flicker !== "yes";
  if (passed) {
    established.push(
      `The ${label} program held a stable static raster for the measured ${formatDuration(heldMs!)} from full-raster-visible.`,
    );
    updates.push({
      claimId,
      status: "verified",
      ...
      metrics: {
        ...(t1 !== null ? { renderLatencyMs: t1 } : {}),
        [VISIBLE_STATIC_HOLD_METRIC]: heldMs!,
      },
    });
    return {
      status: "passed",
      ...
      claimUpdates: updates,
      nextHint:
        "Characterize the raw pixel channels next; strategy viability is derived from the full requirement set.",
    };
  }
  if (initial === "no") {
    rejected.push(
      `The ${label} program did not initially render the raster correctly.`,
    );
    updates.push({
      claimId,
      status: "rejected",
      summary: `The ${label} program failed to render the raster correctly.`,
    });
    return {
      status: "failed",
      established,
      rejected,
      unknowns,
      summary: `The ${label} render failed.`,
      claimUpdates: updates,
      ...(variant === "single"
        ? {
            nextHint:
              "Optionally try the separate two-identical-frame variant.",
          }
        : {}),
    };
  }
  if (still === "no" || flicker === "yes") {
    rejected.push(
      `The ${label} raster did not remain visually stable${heldMs !== null ? ` (visible static hold ${formatDuration(heldMs)}, measured)` : ""}.`,
    );
    updates.push({
      claimId,
      status: "rejected",
      summary: `The ${label} raster rendered but did not remain stable (movement or flicker observed${heldMs !== null ? `; measured hold ${formatDuration(heldMs)}` : ""}).`,
      ...(heldMs !== null
        ? { metrics: { [VISIBLE_STATIC_HOLD_METRIC]: heldMs } }
        : {}),
    });
    return {
      ...
      rejected,
      unknowns,
      summary: `The ${label} raster rendered but was not stable.`,
      claimUpdates: updates,
      ...(variant === "single"
        ? {
            nextHint:
  ...
  if (moved === "no" && heldMs !== null) {
    ...
    unknowns.push(
      `Static for the measured ${formatDuration(heldMs)} — below the ${MINIMUM_STATIC_HOLD_MS / 1000}s window; stability stays unverified.`,
    );
    updates.push({
      claimId,
      status: "unresolved",
      summary: `The ${label} raster stayed still for the measured ${formatDuration(heldMs)}; observation stopped before the required ${MINIMUM_STATIC_HOLD_MS / 1000}s window.`,
      metrics: { [VISIBLE_STATIC_HOLD_METRIC]: heldMs },
    });
    return {
      status: "inconclusive",
      ...
      rejected,
      unknowns,
      summary: `Observation ended after ${formatDuration(heldMs)} — before the required window.`,
      claimUpdates: updates,
    };
  }
  ...
  updates.push({
    claimId,
    status: "unresolved",
    summary: `Observation of the ${label} program was inconclusive.`,
  });
  return {
    status: "inconclusive",
    resolution: "retryable-incomplete",
    ...
    rejected,
    unknowns,
    summary: "The observation was inconclusive.",
    claimUpdates: updates,
  };
}
...
const animationStaticTest: GuidedTestDefinition = {
  ...
  about: {
    ...
    whyRelevant:
      "The static-image (Graffiti) path moved unexpectedly on this display, but animations played perfectly. If one animation frame stays still, it becomes the reliable way to show images.",
    whatMatrixSmithDoes:
      "Uploads the same logical diagnostic raster compiled as a one-frame tiled Animation program with a true-black background.",
    whatChangesOnDevice:
      "The stored display program is replaced with the diagnostic raster.",
    estimatedObservationTime: "Watch for at least 15 seconds.",
    possibleOutcomes: [
      {
        outcome: "The image is correct and stays still",
        learns:
          "A validated static-image strategy exists; images and text can route through it.",
      },
      {
        outcome: "It renders but flickers or resets",
        learns:
          "Single-frame animation playback has a quirk; the separate two-identical-frame variant can be tried.",
      },
      {
        outcome: "It renders incorrectly",
        learns:
          "Single-frame animation decoding differs from multi-frame decoding.",
      },
    ],
    observeInstructions:
      "Watch the panel for at least 15 seconds and answer each question.",
    technicalDetails: [
  ...
  operation: {
    type: "ShowDiagnostic",
    diagnosticId: "animation-static-raster",
    parameters: { frames: 1 },
  },
...
const animationStaticPairTest: GuidedTestDefinition = {
  ...
  prerequisites: [{ claimId: "animation.frames", anyOf: ["verified"] }],
  ...
  about: {
    question:
      "Does repeating the same frame twice make the animation-path still image stable?",
    whyRelevant:
      "A follow-up for when the one-frame variant behaved unexpectedly: some firmware handles a two-frame loop more reliably than a single frame.",
    whatMatrixSmithDoes:
      "Uploads the same raster as TWO identical animation frames per tile.",
    whatChangesOnDevice:
      "The stored display program is replaced with the diagnostic raster.",
    estimatedObservationTime: "Watch for at least 15 seconds.",
    possibleOutcomes: [
      {
        outcome: "The image is correct and stays still",
        learns:
          "The identical-pair variant becomes the validated static strategy.",
      },
      {
        outcome: "Still unstable",
        learns: "The instability is not specific to single-frame programs.",
      },
    ],
    observeInstructions:
      "Watch the panel for at least 15 seconds and answer each question.",
    technicalDetails: [
      "Identical to the one-frame variant except each tile carries the frame twice with 1000 ms delays.",
    ],
  },
  operation: {
    type: "ShowDiagnostic",
...
function patchFieldId(word: number): string {
  return `patch-0x${word.toString(16).padStart(4, "0")}`;
}
...
const pixelChannelTest: GuidedTestDefinition = {
  ...
  about: {
    question:
      "Which parts of the 16-bit pixel value drive which physical LED channels — and does the unused high nibble drive anything?",
    whyRelevant:
      '"White" currently looks tinted on this panel, and the LED package may contain a dedicated white emitter the current encoding never uses. Small fixed patches answer both questions safely.',
    whatMatrixSmithDoes:
      "Uploads eleven small labeled patches over a black background via the verified animation path. Each patch carries one exact raw pixel value; an on-screen diagram maps every patch position to its value.",
    whatChangesOnDevice:
      "The stored display program is replaced with the patch pattern.",
    estimatedObservationTime: "About 2–3 minutes — one color per patch.",
    possibleOutcomes: [
      {
        outcome:
          "Patches match the expected red/green/blue mapping and high-nibble patches stay dark",
        learns:
          "The RGB444 hypothesis is confirmed and the high nibble is ignored.",
      },
      {
        outcome: "A high-nibble patch lights up",
        learns:
          "A fourth physical channel exists; its color and behavior get recorded.",
      },
      {
        outcome: "Colors are permuted",
        learns:
          "The actual channel ordering is characterized and the encoder can be corrected.",
      },
    ],
    observeInstructions:
      "Match each lit patch on the panel to its position in the diagram and pick the color you see. Patches expected to be dark matter too — record them as Off.",
    technicalDetails: [
      "Animation path (verified frames/tiling/black), one frame, 60 s declared delay.",
      "Raw words in patch order: " +
        PIXEL_CHANNEL_PROBE_WORDS.map(
          (word) => `0x${word.toString(16).padStart(4, "0").toUpperCase()}`,
        ).join(", ") +
        ".",
  ...
  observation: [
    ...
    ...PIXEL_CHANNEL_PROBE_WORDS.map((word): ObservationFieldSpec => ({
      kind: "choice",
      id: patchFieldId(word),
      regionId: pixelChannelZoneId(word),
    ...
    })),
    {
      ...
      id: "note",
      prompt:
        "Relative brightness differences or anything else worth recording?",
    },
  ],
  showRegionDiagram: true,
  interpret(values) {
    const answer = (word: number): string | null =>
      choiceAnswer(values, patchFieldId(word));
    const updates: ClaimUpdate[] = [];
    const established: string[] = [];
    const rejected: string[] = [];
    const unknowns: string[] = [];
    ...
    if (zero && zero !== "off")
      rejected.push(
        `The raw 0x0000 patch was not off (${zero}); channel conclusions below are less reliable.`,
      );
    const rgbAsHypothesized =
      red === "red" && green === "green" && blue === "blue";
    const allRgbAnswered = red !== null && green !== null && blue !== null;
    if (rgbAsHypothesized) {
      established.push(
        "Raw 0x0F00 → red, 0x00F0 → green, 0x000F → blue: the RGB444 nibble mapping is confirmed on this panel.",
      );
      updates.push({
        claimId: "pixel.channel-map",
        status: "verified",
        summary:
          "RGB444 mapping physically confirmed: byte0 low nibble = R, byte1 high nibble = G, byte1 low nibble = B.",
        details: {
      ...
      established.push(
        "MatrixSmith's encoder already assumes this mapping: logical RGB values drive the intended channels.",
      );
      updates.push({
        claimId: "pixel.encoder-correctness",
        status: "verified",
        summary:
          "The observed raw mapping matches the RGB444 ordering the encoder emits; logical colors reach the intended physical channels.",
      });
    } else if (allRgbAnswered) {
      const observed = `0x0F00→${red}, 0x00F0→${green}, 0x000F→${blue}`;
      established.push(
        `Observed single-nibble mapping: ${observed}. The raw channel arrangement is now CHARACTERIZED.`,
      );
      rejected.push(
        "The hypothesized RGB444 ordering does not match the observed colors: MatrixSmith's current encoder maps logical channels incorrectly on this panel.",
      );
      ...
      updates.push({
        claimId: "pixel.channel-map",
        status: "verified",
        summary: `Raw channel mapping characterized as ${observed} — a permutation of the RGB444 hypothesis.`,
        details: { observedMap: observed, matchesRgb444: false },
      });
      updates.push({
        claimId: "pixel.encoder-correctness",
        status: "rejected",
        summary: `Encoder mismatch: MatrixSmith emits RGB444 ordering but the panel maps ${observed}. Driver/profile correction required, then a re-verification run. Normal image/text rendering must stay gated until then.`,
        details: {
          observedMap: observed,
          expectedMap: "0x0F00→red, 0x00F0→green, 0x000F→blue",
        },
      });
    } else {
      unknowns.push(
        "Not all single-channel patches were observed; the channel map stays uncharacterized.",
      );
      updates.push({
        claimId: "pixel.channel-map",
        status: "unresolved",
        summary: "Single-channel patch observations were incomplete.",
      });
    }
    const litHighNibble = HIGH_NIBBLE_WORDS.filter((word) => {
      const seen = answer(word);
      return seen !== null && seen !== "off";
    });
    const answeredHighNibble = HIGH_NIBBLE_WORDS.filter(
      (word) => answer(word) !== null,
    );
    if (litHighNibble.length > 0) {
      const details = litHighNibble
        .map(
          (word) =>
            `0x${word.toString(16).padStart(4, "0").toUpperCase()}→${answer(word)}`,
        )
        .join(", ");
      established.push(
        `High-nibble-only values produced light while all RGB bits were zero: ${details}. Strong evidence for a fourth physical channel.`,
      );
      updates.push({
        claimId: "pixel.fourth-channel",
        status: "verified",
        summary: `High-nibble-only words lit (${details}); a fourth physical channel EXISTS. This does not by itself make the profile RGBW, and colorModeRaw=3 semantics remain unknown.`,
        details: { observedAppearance: details },
      });
      ...
      const allWhite = whiteAnswers.every(
        (seen) => seen === "neutral-white" || seen === "tinted-white",
      );
      if (allWhite) {
        updates.push({
          claimId: "pixel.white-channel",
          status: "verified",
          summary: `The lit fourth channel appears white in every observation (${details}).`,
        });
      } else {
        updates.push({
          claimId: "pixel.white-channel",
          status: "unresolved",
          summary: `A fourth channel exists but its appearance (${details}) is not established as white.`,
        });
      }
    } else if (answeredHighNibble.length === HIGH_NIBBLE_WORDS.length) {
      established.push(
        "Every high-nibble-only patch stayed off: the high nibble does not drive a physical emitter at these values.",
      );
      updates.push({
        claimId: "pixel.fourth-channel",
        status: "rejected",
        summary:
          "All high-nibble-only patches (0x1000…0xF000) observed off; no fourth channel is driven by the high nibble.",
      });
      updates.push({
        claimId: "pixel.white-channel",
        status: "rejected",
        summary:
          "No fourth channel exists, so no dedicated white channel exists.",
      });
    } else {
      unknowns.push(
        "High-nibble patches were not all observed; the fourth-channel hypothesis stays open.",
      );
      updates.push({
        claimId: "pixel.fourth-channel",
        status: "unresolved",
        summary: "High-nibble patch observations were incomplete.",
      });
    }
    if (rgbMax === "tinted-white")
      established.push(
        "RGB-max renders tinted rather than neutral white — matching earlier sessions.",
      );
    if (rgbMax === "neutral-white")
      established.push("RGB-max renders neutral white on this observation.");
    const status: GuidedTestInterpretation["status"] =
      rgbAsHypothesized || allRgbAnswered
        ? litHighNibble.length > 0 ||
          answeredHighNibble.length === HIGH_NIBBLE_WORDS.length
          ? "passed"
          : "partial"
        : "inconclusive";
    return {
      ...
      summary:
        status === "passed"
          ? "Pixel-channel behavior characterized."
          : status === "partial"
            ? "Channel mapping characterized; the high nibble needs more observation."
            : "Channel observations were incomplete.",
      claimUpdates: updates,
      nextHint:
        "Run the color/white characterization to compare white options now that the channels are mapped.",
    };
  },
};
...
const colorWhiteTest: GuidedTestDefinition = {
  ...
  prerequisites: [{ claimId: "pixel.channel-map", anyOf: ["verified"] }],
  requiresCompletedTests: [],
  risk: "persistent",
  persistence: "persistent",
  consequence: PERSISTENT_CONSEQUENCE,
  about: {
    question:
      "Do rendered colors — especially white — look visually correct on this panel?",
    whyRelevant:
      "With the wire channels mapped, the remaining question is visual quality: whether RGB-max white is acceptable and how the alternatives compare.",
    whatMatrixSmithDoes:
      "Uploads labeled bands of pure red, green, blue, and RGB-max white via the animation path. When earlier evidence has established a fourth (high-nibble) channel on this device, two additional bands (raw 0xF000 and 0xFFFF) compare it directly; otherwise no meaningless extra regions are shown.",
    whatChangesOnDevice:
      "The stored display program is replaced with the color bands.",
    estimatedObservationTime: "About 1–2 minutes.",
    possibleOutcomes: [
      {
        outcome: "Colors and white look right",
        learns: "No calibration work is needed.",
      },
      {
        outcome: "White looks tinted or channels look imbalanced",
        learns:
          "The imbalance is recorded as structured evidence for a future calibration step — no gain magic is applied blindly.",
      },
    ],
    observeInstructions:
      "Compare each band against its label and judge the white quality.",
    technicalDetails: [
  ...
  operation: {
    type: "ShowDiagnostic",
    diagnosticId: "color-white-probe",
    parameters: { includeHighNibble: 0 },
  },
  ...
  buildOperation: (context) => ({
    ...
    diagnosticId: "color-white-probe",
    parameters: {
      includeHighNibble: operationalTrust(
        "pixel.fourth-channel",
        context.evidence,
      ).trusted
        ? 1
  ...
  observation: [
    {
      ...
      id: "channels-correct",
      regionId: "band-red",
      prompt: "Do the red, green, and blue bands show those exact colors?",
    },
    {
      kind: "choice",
      id: "white-quality",
      regionId: "band-rgb-white",
      prompt: "How does this zone look?",
      options: [
        { id: "neutral-white", label: "Neutral white" },
        { id: "tinted-white", label: "Tinted white" },
        { id: "not-white", label: "Not white at all" },
      ],
      allowOther: true,
    },
    {
      kind: "boolean",
      id: "imbalance",
      prompt: "Is any channel obviously brighter or dimmer than the others?",
      required: false,
    },
    {
      kind: "choice",
      id: "high-nibble-band",
      regionId: "band-extra-channel",
      prompt: "What color is this zone on the display?",
      required: false,
      options: [...COLOR_CHOICES],
      allowOther: true,
  ...
  interpret(values) {
    ...
    if (channels === "yes")
      established.push(
        "Pure red, green, and blue bands rendered their labeled colors.",
      );
    if (channels === "no")
      rejected.push("At least one pure-channel band rendered the wrong color.");
    if (imbalance === "yes")
      established.push("A visible channel brightness imbalance was recorded.");
    if (highNibbleBand)
      established.push(
        `High-nibble band (raw 0xF000) observed as: ${highNibbleBand}.`,
      );
    if (combinedBand)
      established.push(
        `Combined band (raw 0xFFFF) observed as: ${combinedBand}.`,
      );
    if (white === "neutral-white" && channels === "yes") {
      updates.push({
        ...
        status: "verified",
        summary:
          "RGB-max white judged neutral and pure channels correct; no calibration needed.",
      });
      return {
        status: "passed",
        established: [...established, "White looks neutral."],
        rejected,
        unknowns,
        summary: "Color rendering looks visually correct.",
        claimUpdates: updates,
      };
    }
    if (white !== null || channels !== null) {
      updates.push({
        claimId: "pixel.color-calibration",
        status: "unresolved",
        summary: `Visual quality recorded: white=${white ?? "unobserved"}, channels-correct=${channels ?? "unobserved"}, imbalance=${imbalance ?? "unobserved"}. Calibration remains open.`,
      });
      return {
        status: "partial",
        established,
        rejected,
        unknowns,
        summary: "Color quality characterized; calibration remains open.",
        claimUpdates: updates,
      };
    }
    ...
    updates.push({
      claimId: "pixel.color-calibration",
      status: "unresolved",
      summary: "No color-quality observations recorded.",
    });
    return {
      status: "inconclusive",
      resolution: "retryable-incomplete",
...
const normalImageSmokeTest: GuidedTestDefinition = {
  ...
  about: {
    question:
      "Does an ordinary image sent the normal way look right on the panel?",
    whyRelevant:
      "The substrate, encoder and strategy have each been verified separately. This is the only test that exercises them together through the exact path Create uses, so it is what turns 'should work' into 'was seen to work'.",
    whatMatrixSmithDoes:
      "Sends a normal still image — the same ShowFrame the Create screen sends — with no diagnostic-specific compilation. The profile's own preferred static strategy decides how it is delivered.",
    whatChangesOnDevice:
      "The stored display program is replaced with the test image.",
    estimatedObservationTime: "About 20 seconds.",
    possibleOutcomes: [
      {
        outcome: "The image appears and stays put",
        learns: "The production image path works on this panel.",
      },
      {
        outcome: "The image appears and then moves",
        learns:
          "The selected static strategy does not hold a still image after all; the strategy selection needs rechecking.",
      },
      {
        outcome: "The image is wrong from the start",
        learns:
          "The encoder or tiling in the normal path needs attention, independent of playback.",
      },
    ],
    observeInstructions:
      "Tap when the image is fully visible, then watch it. Tap if anything moves, or stop once it has stayed completely still for 15 seconds.",
    technicalDetails: [
  ...
  buildOperation: ({ profile }) => ({
    type: "ShowFrame",
    frame: orientationPattern(profile.width, profile.height),
  }),
  observation: [
    {
      kind: "boolean",
      id: "initial-correct",
      prompt: "Did the image appear correctly?",
    },
    {
      kind: "duration",
      id: "image-visible",
      prompt: "When was the image fully visible? (measured by the timer)",
      required: false,
    },
    {
      kind: "boolean",
      id: "moved",
      prompt: "Did it start moving at any point?",
      required: false,
    },
    {
      kind: "duration",
    ...
    },
    {
      kind: "boolean",
      id: "background-off",
      prompt: "Was the black background genuinely off — not glowing or tinted?",
    },
    {
      kind: "boolean",
      id: "layout-correct",
      prompt:
        "Was the image the right way up, with no gaps or steps between sections?",
    },
    { kind: "note", id: "note", prompt: "Anything else worth recording?" },
  ],
  timer: STATIC_TIMELINE,
  ...
  interpret(values) {
    ...
    if (initial === "no") {
      ...
      updates.push({
        claimId: "image.rendering",
        status: "rejected",
        summary:
          "A normal ShowFrame through the profile's selected static strategy did not render correctly on this panel.",
      });
      return {
        status: "failed",
        established,
        rejected,
        unknowns,
        summary: "The normal image path did not render correctly.",
        claimUpdates: updates,
      };
    }
    if (moved === "yes") {
      ...
      updates.push({
        ...
        status: "rejected",
        summary:
          "A normal ShowFrame rendered but did not stay still, so the selected static strategy does not deliver a stable image here.",
      });
      return {
        status: "partial",
        established,
        ...
        unknowns,
        summary:
          "The normal image rendered, then moved. The selected static strategy needs rechecking.",
        claimUpdates: updates,
      };
    }
    ...
    established.push(
      `The normal image rendered correctly and stayed completely still for ${approximateSeconds(heldMs)}.`,
    );
    updates.push({
      claimId: "image.rendering",
      status: "verified",
    ...
    if (backgroundOff === "yes") established.push("Black was genuinely off.");
      ...
      unknowns.push(
        "Black rendered as something other than off on the normal path; the profile's black semantics need rechecking.",
      );
    }
    if (layout === "yes")
      established.push("Orientation and tiling were correct.");
    else if (layout === "no") {
      ...
      updates.push({
        ...
        status: "unresolved",
        summary:
          "Orientation or tiling was reported wrong on the normal image path; geometry needs recharacterization.",
      });
    }
    ...
    return {
      ...
      claimUpdates: updates,
      nextHint: clean
        ? "The production image path is confirmed; image.rendering can be promoted to observed evidence in the profile."
        : undefined,
    };
  },
};

export const ILEDHAT_GUIDED_TESTS: readonly GuidedTestDefinition[] =
  Object.freeze([
    graffitiBlackTest,
    graffitiTimingTest,
    graffitiStayTimeTest,
    animationStaticTest,
    animationStaticPairTest,
    pixelChannelTest,
    colorWhiteTest,
...
export const COOLLEDUX_GENERIC_GUIDED_TESTS: readonly GuidedTestDefinition[] =
  Object.freeze([]);
...
export function coolLedUxGuidedTests(
  profile: DeviceProfile,
): readonly GuidedTestDefinition[] {
