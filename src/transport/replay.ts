...
export class ReplayTransport implements MatrixTransport {
  ...
  constructor(
    ...
    notifications: readonly Uint8Array[] = [],
  ) {
  ...
  async selectAndConnect(
    ...
    this.connectionId = `replay:${globalThis.crypto?.randomUUID?.() ?? Date.now()}`;
  ...
  async subscribe(
    ...
    for (const bytes of this.notifications)
      queueMicrotask(() => listener(bytes.slice()));
  ...
  async write(
    ...
    throw new Error(
      "Imported and replay diagnostics are offline evidence and cannot transmit.",
    );
  }
}
