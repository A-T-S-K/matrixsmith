...
export class WebBluetoothTransport implements MatrixTransport {
  ...
  async selectAndConnect(options: DeviceSelectionOptions): Promise<DeviceFingerprint> {
    ...
    try {
      ...
      const requestOptions: RequestDeviceOptions & { optionalManufacturerData?: number[] } = options.mode === "inspection"
      ...
      if (manufacturerData && manufacturerData.length > 0) requestOptions.optionalManufacturerData = [...manufacturerData];
   ...
   */
  async reconnectAuthorized(deviceId: string, options: DeviceSelectionOptions): Promise<DeviceFingerprint> {
    const bluetooth = navigator.bluetooth as Bluetooth & { getDevices?: () => Promise<readonly BluetoothDevice[]> };
    if (!bluetooth?.getDevices) throw new Error("This browser does not support reconnecting previously authorized devices.");
    if (this.#state !== "idle" && this.#state !== "error") throw new Error(`Cannot connect while transport is ${this.#state}.`);
    this.#setState("selecting");
    this.trace.record("device.selection.started", { mode: options.mode, reconnect: true });
    try {
      const devices = await bluetooth.getDevices();
      const device = devices.find((candidate) => candidate.id === deviceId);
      if (!device) throw new Error("The previously authorized device was not found. Use the device chooser instead.");
      return await this.#connectDevice(device, options, [...options.hints.optionalServices.map(String)]);
    } catch (error) {
      this.trace.record("error", { scope: "connect", message: errorMessage(error) });
      await this.#cleanup(false);
      this.#setState("error");
  ...
  async #connectDevice(device: BluetoothDevice, options: DeviceSelectionOptions, optionalServices: readonly string[]): Promise<DeviceFingerprint> {
    this.#device = device;
   ...
   */
  async observeAdvertisements(timeoutMs = 4000): Promise<import("../core/device").AdvertisementObservation | null> {
    const device = this.#device as (BluetoothDevice & { watchAdvertisements?: (options?: { signal?: AbortSignal }) => Promise<void> }) | null;
    if (!device?.watchAdvertisements) return null;
    const abort = new AbortController();
    try {
      const observation = await new Promise<import("../core/device").AdvertisementObservation | null>((resolve) => {
        const timer = setTimeout(() => resolve(null), timeoutMs);
        const onAdvertisement = (event: Event): void => {
          clearTimeout(timer);
          resolve(structureAdvertisementEvent(event as Event & Partial<BluetoothAdvertisingEvent>));
        };
        device.addEventListener("advertisementreceived", onAdvertisement, { once: true });
        device.watchAdvertisements!({ signal: abort.signal }).catch(() => { clearTimeout(timer); resolve(null); });
      });
      if (observation && this.#fingerprint) {
        this.#fingerprint = { ...this.#fingerprint, advertisementObservation: observation };
        this.trace.record("advertisement.observed", { rssi: observation.rssi ?? null, manufacturerEntries: observation.manufacturerData.length });
      }
      return observation;
    } catch {
      return null;
...
function structureAdvertisementEvent(event: Event & Partial<BluetoothAdvertisingEvent>): import("../core/device").AdvertisementObservation {
  const manufacturerData: { companyId: number; dataHex: string }[] = [];
  if (event.manufacturerData) for (const [companyId, view] of event.manufacturerData) manufacturerData.push({ companyId, dataHex: viewHex(view) });
  const serviceData: { uuid: string; dataHex: string }[] = [];
  if (event.serviceData) for (const [uuid, view] of event.serviceData) serviceData.push({ uuid, dataHex: viewHex(view) });
  return {
