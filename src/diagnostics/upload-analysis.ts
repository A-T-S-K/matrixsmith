...
export type NotificationDecoder = (
  bytes: Uint8Array,
...
export function analyzeStoredProgramUpload(
  ...
  const txPackets = transaction.packets.filter(
    (packet) => packet.direction === "TX",
  );
  const rxPackets = transaction.packets.filter(
    (packet) => packet.direction === "RX",
  );
  const decoded = rxPackets.map((packet) => ({
    packet,
    notification: decodeHex(packet.hex, decode),
  }));
  ...
  for (const { notification } of decoded) {
    ...
    if (notification.kind === "program-announce-receipt") {
      ...
      if (typeof notification.status === "number")
        rawStatuses.add(notification.status);
    } else if (notification.kind === "program-chunk-receipt") {
      const index =
        typeof notification.fields.chunkIndex === "number"
          ? notification.fields.chunkIndex
          : null;
      if (index !== null)
        (index >= 0 && index < chunkCount
          ? observedChunkIndices
          : unexpectedChunkIndices
        ).push(index);
      if (typeof notification.status === "number")
        rawStatuses.add(notification.status);
    } else {
  ...
  for (let index = 0; index < chunkCount; index += 1)
    if (!observedSet.has(index)) missingChunkIndices.push(index);

  return {
    receipts: {
      expectedChunks: chunkCount,
      announceReceiptsObserved,
      chunkReceiptsObserved:
        observedChunkIndices.length + unexpectedChunkIndices.length,
      observedChunkIndices,
      missingChunkIndices,
      unexpectedChunkIndices,
...
function timingSummary(
  txPackets: readonly TransactionPacket[],
  rxPackets: readonly TransactionPacket[],
): UploadTimingSummary {
  const gaps = txPackets
    .map((packet) => packet.gapSincePreviousTxMs)
  ...
  const totalUploadMs =
    firstTx && lastAccepted
      ? Math.max(0, Date.parse(lastAccepted) - Date.parse(firstTx.timestamp))
      : null;
  const requestedPacing =
    txPackets.find(
      (packet) =>
        typeof packet.scheduledDelayMs === "number" &&
        packet.scheduledDelayMs > 0,
    )?.scheduledDelayMs ?? null;
  const firstRxAfterFinal = lastAccepted
    ? (rxPackets
        .map((packet) => Date.parse(packet.timestamp))
        .filter(
          (time) => Number.isFinite(time) && time >= Date.parse(lastAccepted),
        )
        .sort((a, b) => a - b)[0] ?? null)
    : null;
  ...
  return {
    ...
    firstReceiptLatencyMs:
      firstRxAfterFinal !== null && lastAccepted
        ? firstRxAfterFinal - Date.parse(lastAccepted)
        : null,
    lastReceiptAt: lastRx,
  };
}

export function describeUploadAnalysis(
  analysis: StoredProgramUploadAnalysis,
): readonly string[] {
  ...
  if (receipts.missingChunkIndices.length > 0)
    lines.push(
      `No receipt observed for chunk index(es) ${formatIndexList(receipts.missingChunkIndices)} — not treated as a transmission failure (receipts are not reliable acknowledgements).`,
    );
  if (receipts.unexpectedChunkIndices.length > 0)
    lines.push(
      `Receipts referenced out-of-range chunk index(es) ${formatIndexList(receipts.unexpectedChunkIndices)}.`,
    );
  if (receipts.rawStatuses.length > 0)
    lines.push(
      `Raw status byte(s) observed: ${receipts.rawStatuses.map((status) => `0x${status.toString(16).padStart(2, "0")}`).join(", ")} (semantics unmapped).`,
    );
  if (timing.totalUploadMs !== null)
    lines.push(
      `Upload: ${timing.packetCount} packet(s) over ${timing.totalUploadMs} ms; TX gaps min/avg/max ${timing.minTxGapMs ?? "—"}/${timing.averageTxGapMs ?? "—"}/${timing.maxTxGapMs ?? "—"} ms (requested pacing ${timing.requestedPacingMs ?? "none"} ms).`,
    );
  if (timing.firstReceiptLatencyMs !== null)
    lines.push(
      `First receipt arrived ${timing.firstReceiptLatencyMs} ms after the final host-accepted write.`,
    );
  return lines;
}
...
function decodeHex(
  ...
  if (
    ...
    return null;
  return decode(
    Uint8Array.from(cleaned.match(/../g) ?? [], (pair) =>
