...
interface SemanticFields { readonly width: number; readonly height: number; readonly labels: Uint8Array; readonly signed: readonly Float32Array[] }
  ...
  for (const phase of phases) {
    ...
    for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) {
      ...
      for (const oy of [0.2, 0.5, 0.8]) for (const ox of [0.2, 0.5, 0.8]) {
        ...
        votes[fieldLabel(fields, sx / source.width, sy / source.height)]! += 1;
      }
      const centerSx = transform.sx + (x + 0.5 + phase - transform.dx) / transform.dw * transform.sw;
      const centerSy = transform.sy + (y + 0.5 - transform.dy) / transform.dh * transform.sh;
      const fieldIndex = fieldOffset(fields, centerSx / source.width, centerSy / source.height);
      const winner = votes.reduce((bestIndex, vote, index) => {
        const score = vote * 2 + Math.max(-2, Math.min(2, (fields.signed[index]?.[fieldIndex] ?? -99) / 2));
        const bestScore = (votes[bestIndex] ?? 0) * 2 + Math.max(-2, Math.min(2, (fields.signed[bestIndex]?.[fieldIndex] ?? -99) / 2));
    ...
    const score = represented * 100 - speckles * 2 - symmetryDrift - fragmentation * 5;
...
function buildSemanticFields(source: DecodedImageSource, palette: readonly PaletteColor[]): SemanticFields {
  ...
  for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) labels[y * width + x] = nearestPalette(source, Math.min(source.width - 1, Math.floor((x + 0.5) / width * source.width)), Math.min(source.height - 1, Math.floor((y + 0.5) / height * source.height)), palette);
  const signed = palette.map((_, label) => {
    const toRegion = chamferDistance(labels, width, height, (value) => value === label);
    const toOther = chamferDistance(labels, width, height, (value) => value !== label);
    const field = new Float32Array(labels.length);
    for (let i = 0; i < field.length; i += 1) field[i] = labels[i] === label ? toOther[i] ?? 0 : -(toRegion[i] ?? 0);
    return field;
  });
  return { width, height, labels, signed };
}

function chamferDistance(labels: Uint8Array, width: number, height: number, target: (label: number) => boolean): Float32Array {
  const distance = new Float32Array(labels.length); const far = width + height; for (let i = 0; i < distance.length; i += 1) distance[i] = target(labels[i] ?? 0) ? 0 : far;
  const root2 = Math.SQRT2;
  for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) { const i = y * width + x; let d = distance[i] ?? far; if (x > 0) d = Math.min(d, (distance[i - 1] ?? far) + 1); if (y > 0) d = Math.min(d, (distance[i - width] ?? far) + 1); if (x > 0 && y > 0) d = Math.min(d, (distance[i - width - 1] ?? far) + root2); if (x + 1 < width && y > 0) d = Math.min(d, (distance[i - width + 1] ?? far) + root2); distance[i] = d; }
  for (let y = height - 1; y >= 0; y -= 1) for (let x = width - 1; x >= 0; x -= 1) { const i = y * width + x; let d = distance[i] ?? far; if (x + 1 < width) d = Math.min(d, (distance[i + 1] ?? far) + 1); if (y + 1 < height) d = Math.min(d, (distance[i + width] ?? far) + 1); if (x + 1 < width && y + 1 < height) d = Math.min(d, (distance[i + width + 1] ?? far) + root2); if (x > 0 && y + 1 < height) d = Math.min(d, (distance[i + width - 1] ?? far) + root2); distance[i] = d; }
  return distance;
}

function fieldOffset(fields: SemanticFields, normalizedX: number, normalizedY: number): number { const x = Math.max(0, Math.min(fields.width - 1, Math.floor(normalizedX * fields.width))); const y = Math.max(0, Math.min(fields.height - 1, Math.floor(normalizedY * fields.height))); return y * fields.width + x; }
function fieldLabel(fields: SemanticFields, normalizedX: number, normalizedY: number): number { return fields.labels[fieldOffset(fields, normalizedX, normalizedY)] ?? 0; }
...
function fragmentationPenalty(frame: Framebuffer, palette: readonly PaletteColor[]): number {
  let penalty = 0;
  for (let label = 1; label < palette.length; label += 1) {
    const color = palette[label]!; const seen = new Uint8Array(frame.width * frame.height); let components = 0;
    for (let y = 0; y < frame.height; y += 1) for (let x = 0; x < frame.width; x += 1) { const start = y * frame.width + x; if (seen[start]) continue; const pixel = frame.getPixel(x, y); if (pixel.r !== Math.round(color.r) || pixel.g !== Math.round(color.g) || pixel.b !== Math.round(color.b)) continue; components += 1; const stack = [start]; seen[start] = 1; while (stack.length) { const at = stack.pop()!; const ax = at % frame.width; const ay = Math.floor(at / frame.width); for (const [ox, oy] of [[-1, 0], [1, 0], [0, -1], [0, 1]] as const) { const nx = ax + ox; const ny = ay + oy; if (nx < 0 || ny < 0 || nx >= frame.width || ny >= frame.height) continue; const ni = ny * frame.width + nx; if (seen[ni]) continue; const next = frame.getPixel(nx, ny); if (next.r === Math.round(color.r) && next.g === Math.round(color.g) && next.b === Math.round(color.b)) { seen[ni] = 1; stack.push(ni); } } } }
