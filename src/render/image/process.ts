...
export function processImage(
  ...
  const resolvedMode: ResolvedImageMode =
    recipe.mode === "auto"
      ? analysis.confidence === "high" && analysis.likelyMode !== "photo"
        ? analysis.likelyMode
        : "photo"
      : recipe.mode;
  ...
  try {
    if (resolvedMode === "artwork") {
      ...
      detail = {
        semanticPaletteSize: result.paletteSize,
        signedDistanceFields: true,
        selectedGridPhase: result.phase,
        hardSemanticColors: true,
      };
    } else if (resolvedMode === "pixel-art")
      intermediate = reducePixelArt(source, width, height, recipe.composition);
    else
      intermediate = reducePhoto(
        source,
        width,
        height,
        recipe.composition,
    ...
    warnings.push(
      `Advanced processing failed; Photo-safe fallback used (${error instanceof Error ? error.message : String(error)}).`,
    );
  }
  ...
  return {
    ...
    analysis,
    resolvedMode,
    resolvedComposition: recipe.composition,
    outputColorCount: countColors(frame),
    warnings,
    processingMetadata: {
      reducer: resolvedMode,
      deviceColorModel: model.id,
      dithering: false,
