...
export function resolveComposition(source: DecodedImageSource, targetWidth: number, targetHeight: number, composition: ImageComposition): SourceTransform {
  ...
  const scale = (cover ? Math.max : Math.min)(targetWidth / effectiveWidth, targetHeight / sh) * clamp(composition.zoom ?? 1, 0.25, 4);
...
function foregroundBounds(source: DecodedImageSource): { x: number; y: number; width: number; height: number } | null {
  let minX = source.width; let minY = source.height; let maxX = -1; let maxY = -1;
  const corner = [source.rgba[0] ?? 0, source.rgba[1] ?? 0, source.rgba[2] ?? 0];
  for (let y = 0; y < source.height; y += 1) for (let x = 0; x < source.width; x += 1) {
    const i = (y * source.width + x) * 4;
    const alpha = source.rgba[i + 3] ?? 0;
    const distance = Math.abs((source.rgba[i] ?? 0) - corner[0]!) + Math.abs((source.rgba[i + 1] ?? 0) - corner[1]!) + Math.abs((source.rgba[i + 2] ?? 0) - corner[2]!);
    if (alpha > 16 && (source.hasAlpha ? alpha > 48 : distance > 24)) { minX = Math.min(minX, x); minY = Math.min(minY, y); maxX = Math.max(maxX, x); maxY = Math.max(maxY, y); }
  }
