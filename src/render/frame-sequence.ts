...
export class FrameSequence {
  ...
  constructor(frames: readonly Framebuffer[], timing: readonly FrameTiming[]) {
    if (frames.length === 0)
      throw new RangeError("A frame sequence needs at least one frame.");
    if (frames.length !== timing.length)
      throw new RangeError("Each frame needs a timing entry.");
    ...
    if (
      ...
      throw new RangeError(
        "All sequence frames must have identical dimensions.",
      );
    }
    if (
      timing.some(
      ...
      throw new RangeError(
        "Frame timings must be positive integer milliseconds.",
      );
    }
