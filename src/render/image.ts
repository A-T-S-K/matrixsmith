...
export function computeFitPlacement(sourceWidth: number, sourceHeight: number, targetWidth: number, targetHeight: number, mode: FitMode): FitPlacement {
  ...
  switch (mode) {
    ...
    case "contain": {
      ...
      return { sx: 0, sy: 0, sw: sourceWidth, sh: sourceHeight, dx: Math.floor((targetWidth - dw) / 2), dy: Math.floor((targetHeight - dh) / 2), dw, dh };
    }
    case "cover": {
      ...
      return { sx: Math.floor((sourceWidth - sw) / 2), sy: Math.floor((sourceHeight - sh) / 2), sw, sh, dx: 0, dy: 0, dw: targetWidth, dh: targetHeight };
    }
    case "center": {
      ...
      return { sx: Math.floor((sourceWidth - sw) / 2), sy: Math.floor((sourceHeight - sh) / 2), sw, sh, dx: Math.floor((targetWidth - sw) / 2), dy: Math.floor((targetHeight - sh) / 2), dw: sw, dh: sh };
    }
  }
}

/** Copy decoded RGBA pixel data into a logical framebuffer (alpha composited over black). */
export function rgbaToFramebuffer(rgba: Uint8ClampedArray, width: number, height: number): Framebuffer {
  if (rgba.length !== width * height * 4) throw new RangeError("RGBA buffer does not match the given dimensions.");
  ...
  for (let index = 0, pixel = 0; index < rgba.length; index += 4, pixel += 1) {
    ...
    frame.setPixel(x, y, Math.round((rgba[index] ?? 0) * alpha), Math.round((rgba[index + 1] ?? 0) * alpha), Math.round((rgba[index + 2] ?? 0) * alpha));
  }
  return frame;
}
 ...
 */
export async function decodeImageFile(file: Blob, targetWidth: number, targetHeight: number, mode: FitMode): Promise<DecodedImage> {
  const bitmap = await createImageBitmap(file);
  try {
    const placement = computeFitPlacement(bitmap.width, bitmap.height, targetWidth, targetHeight, mode);
    const canvas = typeof OffscreenCanvas !== "undefined" ? new OffscreenCanvas(targetWidth, targetHeight) : createDomCanvas(targetWidth, targetHeight);
    ...
    context.drawImage(bitmap, placement.sx, placement.sy, placement.sw, placement.sh, placement.dx, placement.dy, placement.dw, placement.dh);
    const rgba = context.getImageData(0, 0, targetWidth, targetHeight).data;
    return { frame: rgbaToFramebuffer(rgba, targetWidth, targetHeight), sourceWidth: bitmap.width, sourceHeight: bitmap.height, fitMode: mode };
  } finally {
    bitmap.close();
  }
}
...
export function readGifMetadata(bytes: Uint8Array): GifMetadata {
  ...
  if (!isGif) return { byteLength: bytes.length, width: null, height: null, isGif: false };
