...
export type TimingMarkSource = "automatic-transport" | "human-observed" | "derived-human-observed";
...
export function validAttempts(attempts: readonly ObservationAttempt[]): readonly ObservationAttempt[] {
  return attempts.filter(isValidAttempt);
}

/** Elapsed for a phase within an attempt, or null when it was never marked. */
export function markElapsed(attempt: ObservationAttempt, event: string): number | null {
...
export function describeMarkSource(source: TimingMarkSource): string {
  switch (source) {
    case "automatic-transport": return "automatically measured";
    case "human-observed": return "human observed";
    case "derived-human-observed": return "derived from human-observed marks";
  }
}
 ...
 * avoid.
 */
export function aggregateAttemptDurations(durations: readonly number[]): AttemptAggregate | null {
  const values = durations.filter((value) => Number.isFinite(value)).slice().sort((a, b) => a - b);
  if (values.length === 0) return null;
  const middle = Math.floor(values.length / 2);
  const representativeMs = values.length % 2 === 1
    ? values[middle]!
    : Math.round((values[middle - 1]! + values[middle]!) / 2);
  return { count: values.length, values, minMs: values[0]!, maxMs: values[values.length - 1]!, representativeMs };
}

export function describeAggregate(aggregate: AttemptAggregate): string {
  if (aggregate.count === 1) return approximateSeconds(aggregate.representativeMs);
  if (aggregate.minMs === aggregate.maxMs) return `${approximateSeconds(aggregate.representativeMs)} across ${aggregate.count} attempts`;
...
export function describeAttempt(attempt: ObservationAttempt): readonly string[] {
  const lines: string[] = [];
  lines.push(`Attempt ${attempt.attemptNumber} — ${attempt.validity === "valid" ? "valid" : "INVALID"}`);
  if (attempt.validity !== "valid") {
    lines.push(`  Reason: ${attempt.invalidationReason ?? ATTEMPT_INVALIDATION_LABELS[attempt.validity]}`);
    lines.push("  Excluded from conclusions.");
  }
  const parameters = Object.entries(attempt.parameters);
  if (parameters.length > 0) lines.push(`  Parameters: ${parameters.map(([key, value]) => `${key}=${value}`).join(", ")}`);
  lines.push(attempt.t0 ? `  T0 upload complete: ${attempt.t0} (automatically measured)` : "  T0: not reached");
  for (const mark of attempt.marks) {
    lines.push(`  ${mark.event}: +${formatDuration(mark.elapsedMs)} (${describeMarkSource(mark.source)})`);
  }
