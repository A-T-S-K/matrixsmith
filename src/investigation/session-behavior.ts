...
export interface SessionResolvedBehavior {
   ...
   */
  readonly observedChannelMap: { readonly map: string; readonly matchesRgb444: boolean } | null;
}

export const EMPTY_SESSION_BEHAVIOR: SessionResolvedBehavior = Object.freeze({ graffitiBlack: null, observedChannelMap: null });

export function resolveSessionBehavior(evidence: readonly ClaimEvidence[]): SessionResolvedBehavior {
  ...
  if (black.trusted && black.basis?.scope === "current-session") {
    ...
    if (zeroBehavior === "true-black") graffitiBlack = { state: "true-black", basis: "observed" };
    else if (zeroBehavior === "white-sentinel") graffitiBlack = { state: "white-sentinel", basis: "observed", workaroundWord: 0x0004 };
  }
  ...
  if (channel.trusted && channel.basis?.scope === "current-session" && typeof channel.basis.details?.observedMap === "string") {
    observedChannelMap = { map: channel.basis.details.observedMap, matchesRgb444: channel.basis.details.matchesRgb444 === true };
  }
