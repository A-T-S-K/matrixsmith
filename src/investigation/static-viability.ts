...
const STRATEGY_REQUIREMENTS: Readonly<
  Record<RasterStrategy, readonly ClaimId[]>
> = Object.freeze({
  graffiti: GRAFFITI_REQUIREMENTS,
  "animation-single-frame": Object.freeze<ClaimId[]>([
    ...ANIMATION_COMMON,
    "animation.static-single-frame",
  ]),
  "animation-identical-frames": Object.freeze<ClaimId[]>([
    ...ANIMATION_COMMON,
...
function requirementState(
  ...
  if (trust.trustedStatus === "rejected") {
    return {
      ...
      state: "failed",
      trustedStatus: trust.trustedStatus,
      detail: trust.basis?.summary ?? "Rejected by trusted evidence.",
    };
  }
  if (trust.trusted) {
    if (HOLD_METRIC_CLAIMS.includes(claimId)) {
      ...
      if (typeof hold === "number" && hold < MINIMUM_STATIC_HOLD_MS) {
        return {
          ...
          state: "open",
          trustedStatus: trust.trustedStatus,
          detail: `Measured visibly-static hold ${Math.round(hold / 100) / 10}s is below the required ${MINIMUM_STATIC_HOLD_MS / 1000}s.`,
        };
      }
    }
    return {
      ...
      state: "met",
      trustedStatus: trust.trustedStatus,
      detail: trust.basis?.summary ?? "Verified by trusted evidence.",
    };
  }
  return {
    claimId,
    ...
    state: "open",
    trustedStatus: trust.trustedStatus,
    detail: trust.basis?.summary ?? "No trusted verification yet.",
  };
}
...
function evaluateStrategy(
  ...
  const requirements = STRATEGY_REQUIREMENTS[strategy].map((claimId) =>
    requirementState(claimId, evidence),
  );
  const failed = requirements.filter(
    (requirement) => requirement.state === "failed",
  );
  const open = requirements.filter(
    (requirement) => requirement.state === "open",
  );
  const verdict: StrategyVerdict =
    failed.length > 0 ? "not-viable" : open.length === 0 ? "viable" : "open";
  const summary =
    verdict === "viable"
...
export function allStrategyRequirementClaims(): ReadonlySet<ClaimId> {
  return new Set(
    STRATEGY_PREFERENCE.flatMap((strategy) => STRATEGY_REQUIREMENTS[strategy]),
  );
}

export function evaluateStaticViability(
  evidence: readonly ClaimEvidence[],
): StaticViabilityAssessment {
  const strategies = STRATEGY_PREFERENCE.map((strategy) =>
    evaluateStrategy(strategy, evidence),
  );
  const byStrategy = new Map(
    strategies.map((entry) => [entry.strategy, entry]),
  );
  const selected =
    STRATEGY_PREFERENCE.find(
      (strategy) => byStrategy.get(strategy)?.verdict === "viable",
    ) ?? null;
  const pursued =
    STRATEGY_PREFERENCE.find(
      (strategy) => byStrategy.get(strategy)?.verdict !== "not-viable",
    ) ?? null;
  const pursuedEntry = pursued ? (byStrategy.get(pursued) ?? null) : null;
  const nextOpenRequirement =
    pursuedEntry?.requirements.find(
      (requirement) => requirement.state === "open",
    )?.claimId ?? null;
  const overall: StrategyVerdict = selected
    ? "viable"
    : strategies.every((entry) => entry.verdict === "not-viable")
      ? "not-viable"
      : "open";
