...
import {
  ...
  type PanelProgramKind,
  type PanelProgramState,
  type ReopenedExperiment,
  type TransferReason,
  type TransferRecord,
} from "./orchestration";
import type {
...
const CLAIM_IDS: ReadonlySet<string> = new Set(
  CLAIM_DEFINITIONS.map((definition) => definition.id),
);
...
const EVIDENCE_SCOPES: readonly string[] = [
  "source-reference",
  "built-in-profile",
  "current-session",
...
const PROVENANCES: readonly string[] = [
  "observed",
  "corroborated",
  "source-derived",
  "inferred",
...
const GUIDED_STATUSES: readonly string[] = [
  "passed",
  "failed",
  "partial",
  "inconclusive",
...
const ATTEMPT_VALIDITIES: readonly string[] = [
  "valid",
  "missed-t1",
  "missed-t2",
  "accidental-tap",
...
const MARK_SOURCES: readonly string[] = [
  "automatic-transport",
  "human-observed",
  "derived-human-observed",
];
const PANEL_KINDS: readonly string[] = [
  "guided-diagnostic",
  "ordinary-content",
  "validation",
  "none",
];
const PANEL_CERTAINTIES: readonly string[] = [
  "known-active",
  "known-replaced",
  "unknown",
];
...
export function sanitizeInvestigation(
  ...
  return {
    ...
    completedTests: value.completedTests
      .map(sanitizeCompletedTest)
      .filter((test): test is CompletedGuidedTest => test !== null),
    claimEvidence: value.claimEvidence
      .map(sanitizeClaimEvidence)
      .filter((entry): entry is ClaimEvidence => entry !== null),
    notes: Array.isArray(value.notes)
      ? value.notes.filter((note): note is string => typeof note === "string")
      : [],
    orchestration: sanitizeOrchestration(
      value.orchestration,
      panelUncertaintyReason,
    ),
  };
}

function sanitizeGoal(value: Record<string, unknown>): InvestigationGoal {
  const kind = GOAL_KINDS.includes(text(value.kind))
    ? (value.kind as InvestigationGoal["kind"])
    : "develop";
  return {
    kind,
    description: text(value.description),
    ...(typeof value.symptomId === "string"
      ? { symptomId: value.symptomId as SymptomId }
      : {}),
  };
}
...
function sanitizeClaimEvidence(value: unknown): ClaimEvidence | null {
  if (!isObject(value)) return null;
  if (typeof value.claimId !== "string" || !CLAIM_IDS.has(value.claimId))
    return null;
  ...
  return {
    ...
    provenance: (PROVENANCES.includes(text(value.provenance))
      ? value.provenance
      : "inferred") as EvidenceProvenance,
    ...
    ...(Array.isArray(value.transactionIds)
      ? {
          transactionIds: value.transactionIds.filter(
...
function sanitizeCompletedTest(value: unknown): CompletedGuidedTest | null {
  ...
  const attempts = Array.isArray(value.attempts)
    ? value.attempts
        .map(sanitizeObservationAttempt)
        .filter((attempt): attempt is ObservationAttempt => attempt !== null)
    : [];
  return {
    testId: value.testId,
    ...
    observations: Array.isArray(value.observations)
      ? value.observations.filter((entry): entry is ObservationValue =>
          isValidObservationValue(entry as ObservationValue),
        )
      : [],
    ...
    ...(isObject(value.parameters) && stringRecord(value.parameters)
      ? { parameters: stringRecord(value.parameters)! }
      : {}),
    ...(attempts.length > 0 ? { attempts } : {}),
    ...(RESOLUTIONS.includes(text(value.resolution))
      ? { resolution: value.resolution as ExperimentResolution }
      : {}),
...
function sanitizeObservationAttempt(value: unknown): ObservationAttempt | null {
  ...
  return {
    ...
    marks: Array.isArray(value.marks)
      ? value.marks
          .map(sanitizeMark)
          .filter((mark): mark is PhysicalTimingMark => mark !== null)
      : [],
    values: Array.isArray(value.values)
      ? value.values.filter((entry): entry is ObservationValue =>
          isValidObservationValue(entry as ObservationValue),
        )
      : [],
...
function sanitizeOrchestration(
  ...
  if (!isObject(value))
    return demoted(emptyOrchestration(), panelUncertaintyReason);
  const orchestration: InvestigationOrchestration = {
    experiments: Array.isArray(value.experiments)
      ? value.experiments
          .map(sanitizeExperiment)
          .filter((run): run is ExperimentRun => run !== null)
      : [],
    ...
    reopened: Array.isArray(value.reopened)
      ? value.reopened
          .map(sanitizeReopened)
          .filter((entry): entry is ReopenedExperiment => entry !== null)
      : [],
    recommendationTrail: Array.isArray(value.recommendationTrail)
      ? value.recommendationTrail
          .map(sanitizeTrailEntry)
          .filter((entry): entry is RecommendationTrailEntry => entry !== null)
      : [],
    cycleVerdict: {
      cycling: Boolean(
        isObject(value.cycleVerdict) && value.cycleVerdict.cycling,
      ),
      testIds: isObject(value.cycleVerdict)
        ? stringArray(value.cycleVerdict.testIds)
        : [],
      detail:
        isObject(value.cycleVerdict) &&
        typeof value.cycleVerdict.detail === "string"
          ? value.cycleVerdict.detail
          : null,
    },
  };
...
function sanitizeExperiment(value: unknown): ExperimentRun | null {
  ...
  if (status !== "in-progress" && !GUIDED_STATUSES.includes(status))
    return null;
  return {
    experimentRunId: value.experimentRunId,
    ...
    corePlanStepId:
      typeof value.corePlanStepId === "string" ? value.corePlanStepId : null,
    variant: typeof value.variant === "string" ? value.variant : null,
    parameters: numberRecord(value.parameters),
    fingerprint,
    status: status as ExperimentRun["status"],
    resolution: RESOLUTIONS.includes(text(value.resolution))
      ? (value.resolution as ExperimentResolution)
      : null,
    startedAt: text(value.startedAt),
    completedAt:
    ...
    attempts: Array.isArray(value.attempts)
      ? value.attempts
          .map(sanitizeExperimentAttempt)
          .filter((attempt): attempt is ExperimentAttempt => attempt !== null)
      : [],
    conclusion: typeof value.conclusion === "string" ? value.conclusion : null,
    reopenReason:
      typeof value.reopenReason === "string" ? value.reopenReason : null,
  };
}
  ...
  return {
    ...
    invalidationReason:
      typeof value.invalidationReason === "string"
        ? value.invalidationReason
        : null,
    failureKind: FAILURE_KINDS.includes(text(value.failureKind))
      ? (value.failureKind as AttemptFailureKind)
      : null,
  };
}
...
function sanitizeTransfer(value: unknown): TransferRecord | null {
  ...
  return {
    ...
    finalWriteAcceptedAt:
      typeof value.finalWriteAcceptedAt === "string"
        ? value.finalWriteAcceptedAt
        : null,
    failureReason:
...
function sanitizeFingerprint(
  value: unknown,
): DiagnosticExecutionFingerprint | null {
  if (
    !isObject(value) ||
  ...
  if (
    basis !== "browser-authorized-device" &&
    basis !== "fingerprint-shape" &&
    basis !== "unidentified"
  )
    return null;
  return {
    physicalDeviceKey:
      typeof value.physicalDeviceKey === "string"
        ? value.physicalDeviceKey
        : null,
    ...
    programCrc32:
      typeof value.programCrc32 === "string" ? value.programCrc32 : null,
    rasterStrategy:
      typeof value.rasterStrategy === "string" ? value.rasterStrategy : null,
    key: value.key,
  };
}
...
function sanitizePanelProgram(value: unknown): PanelProgramState {
  ...
  if (
    !PANEL_KINDS.includes(text(value.kind)) ||
    !PANEL_CERTAINTIES.includes(text(value.certainty))
  )
    return UNKNOWN_PANEL_PROGRAM;
  return {
    ...
    uncertaintyReason:
      typeof value.uncertaintyReason === "string"
...
function sanitizeTrailEntry(value: unknown): RecommendationTrailEntry | null {
  ...
  return {
    ...
    ...(ORIGINS.includes(text(value.origin))
      ? { origin: value.origin as RecommendationOrigin }
      : {}),
  };
}
