...
export class TransferWakeLock {
  ...
  #visibility?: () => void;
  constructor(
    private readonly navigatorLike: WakeLockNavigator,
    private readonly documentLike?: Pick<
      Document,
      "visibilityState" | "addEventListener" | "removeEventListener"
    >,
  ) {}
  get supported(): boolean {
    return Boolean(this.navigatorLike.wakeLock?.request);
  }
  async acquire(): Promise<boolean> {
    this.#active = true;
    if (!this.#visibility && this.documentLike) {
      this.#visibility = () => {
        if (
          this.#active &&
          this.documentLike?.visibilityState === "visible" &&
          !this.#handle
        )
          void this.#request();
      };
      this.documentLike.addEventListener("visibilitychange", this.#visibility);
    }
    return this.#request();
  }
  ...
  async #request(): Promise<boolean> {
    if (!this.#active || !this.navigatorLike.wakeLock || this.#handle)
      return Boolean(this.#handle);
    try {
      const handle = await this.navigatorLike.wakeLock.request("screen");
      if (!this.#active) {
